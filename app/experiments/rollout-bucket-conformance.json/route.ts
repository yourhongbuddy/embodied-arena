import { experimentRolloutBucketConformanceContract } from "../rollout-bucket-conformance.ts";
import {
  experimentRolloutBucketConformanceVerifierContract,
  experimentRolloutBucketConformanceVerifierSource,
} from "../rollout-bucket-conformance-verifier-source.ts";
...
export async function GET() {
  const source_sha256 = hex(await crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(experimentRolloutBucketConformanceVerifierSource),
  ));
