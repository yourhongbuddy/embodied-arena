...
export function ExperimentDashboard(){
  ...
  return <div className="experimentSurface">
    ...
    <section className="experimentResults">
      ...
      <aside><b>READING THE TEST</b><p>The randomization and analysis unit is one experiment-scoped anonymous browser ID. Cross-variant units ({data?.cross_variant_units_excluded??0}) and same-variant units with multiple tokens ({data?.multi_token_units_excluded??0}) are excluded and disclosed for this window. Repeat sessions and receipts for the same unit and token are deduplicated. The assignment seed remains on-device; the experiment-only ID is not shared across experiments, and first-party analytics uses a rolling 35-day raw-event window. Control comparisons use Newcombe–Wilson intervals with Bonferroni control across two comparisons. Signals are descriptive and never auto-declare a winner. Preview and operator traffic are excluded.</p></aside>
    </section>
  </div>
}
