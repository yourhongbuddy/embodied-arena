...
type Data={status:"ready"|"unavailable";experiment:string;window_days:number;primary_goal:string;total_exposed_sessions:number;cross_variant_sessions_excluded:number;multi_token_sessions_excluded:number;sample_ratio_mismatch:RatioCheck;variants:Row[];comparisons:Comparison[]};
...
export function ExperimentDashboard(){
  ...
  return <div className="experimentSurface">
    ...
    <section className="experimentResults">
      ...
      <aside><b>READING THE TEST</b><p>Each counted session must have exactly one assigned version and one exposure token. Cross-variant sessions ({data?.cross_variant_sessions_excluded??0}) and same-variant sessions with multiple tokens ({data?.multi_token_sessions_excluded??0}) are excluded and disclosed for this window. Repeat receipts for the same token are deduplicated. Control comparisons use Newcombe–Wilson intervals with Bonferroni control across two comparisons. Signals are descriptive and never auto-declare a winner. Preview and operator traffic are excluded.</p></aside>
    </section>
  </div>
}
