...
function validate(records) {
  ...
  for (const [index, record] of records.entries()) {
    ...
    if (typeof record.environment !== "string" || record.environment.length === 0 || ids.has(record.environment)) return fail("invalid_records", "environment identifiers must be non-empty and unique");
    ...
    if (record.disposition === "completed" && record.resident_hours !== HORIZON) return fail("invalid_records", "completion requires exactly 10000 hours");
