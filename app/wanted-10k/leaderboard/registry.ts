...
export function validateRegistryEntry(entry: RegistryEntry) {
  ...
  if (entry.root_envelope_integrity_verified !== true || !Number.isInteger(entry.root_envelope_count) || entry.root_envelope_count < entry.independent_environments) errors.push("A ranked row requires passing 0.2-RE1 root-envelope integrity with at least one verified signed-prefix envelope per environment.");
