...
export type RegistryEntry = {
  ...
  site_heterogeneity_verified: true;
  site_count: number;
  ...
  site_leave_one_out_unidentifiable: number;
  total_resident_hours: number;
  root_envelope_integrity_verified: true;
  root_envelope_verification_profile: "0.2-REB1";
  root_envelope_count: number;
  ...
  sampling_stopping_integrity_verified: true;
  sampling_planned_units: number;
  sampling_actual_units: number;
  sampling_operational_overshoot_units: number;
  sampling_planned_exposure_hours: number;
  sampling_actual_exposure_hours: number;
  sampling_primary_outcome_access_before_cutoff: 0;
  sampling_unscheduled_primary_analyses: 0;
  sampling_result_informed_extensions: 0;
  sampling_result_informed_early_stops: 0;
  sampling_target_changes_after_activity: 0;
  sampling_cutoff_changes_after_activity: 0;
  sampling_excluded_activated_units: 0;
  sampling_excluded_overshoot_units: 0;
...
export function validateRegistryEntry(entry: RegistryEntry) {
  ...
  if (entry.root_envelope_integrity_verified !== true || entry.root_envelope_verification_profile !== "0.2-REB1" || !Number.isInteger(entry.root_envelope_count) || entry.root_envelope_count < entry.independent_environments) errors.push("A ranked row requires passing 0.2-REB1 batch verification of 0.2-RE1 root-envelope integrity with at least one verified signed-prefix envelope per environment.");
  const siteWantedIdentifiable = entry.identifiable_site_count > 0 && numeric(entry.site_wanted_min) && numeric(entry.site_wanted_max) && numeric(entry.site_wanted_range) && entry.site_wanted_min >= 0 && entry.site_wanted_min <= entry.site_wanted_max && entry.site_wanted_max <= 100 && Math.abs(entry.site_wanted_range - (entry.site_wanted_max - entry.site_wanted_min)) < .000001;
  const siteWantedUnidentifiable = entry.identifiable_site_count === 0 && entry.site_wanted_min === null && entry.site_wanted_max === null && entry.site_wanted_range === null;
  const siteLeaveOneOut = (entry.site_leave_one_out_unidentifiable === entry.site_count && entry.leave_one_site_out_maximum_absolute_shift === null) || (entry.site_leave_one_out_unidentifiable < entry.site_count && numeric(entry.leave_one_site_out_maximum_absolute_shift) && entry.leave_one_site_out_maximum_absolute_shift >= 0 && entry.leave_one_site_out_maximum_absolute_shift <= 100);
  const siteHeterogeneityDisclosure = entry.site_heterogeneity_verified === true && Number.isInteger(entry.site_count) && entry.site_count >= 3 && Number.isInteger(entry.smallest_site_n) && entry.smallest_site_n >= 3 && entry.site_count * entry.smallest_site_n <= entry.independent_environments && numeric(entry.maximum_site_share) && entry.maximum_site_share > 0 && entry.maximum_site_share <= .5 && Number.isInteger(entry.identifiable_site_count) && entry.identifiable_site_count >= 0 && entry.identifiable_site_count <= entry.site_count && Number.isInteger(entry.site_leave_one_out_unidentifiable) && entry.site_leave_one_out_unidentifiable >= 0 && entry.site_leave_one_out_unidentifiable <= entry.site_count && (siteWantedIdentifiable || siteWantedUnidentifiable) && siteLeaveOneOut;
  if (!siteHeterogeneityDisclosure) errors.push("A ranked row requires a coherent passing 0.2-SH1 site-heterogeneity disclosure with at least three substantive sites and no site above 50% of the cohort.");
  ...
  const endpointDisclosure = entry.endpoint_adjudication_verified === true && [entry.endpoint_voluntary_rejections,entry.endpoint_administrative_completions,entry.endpoint_censors,entry.horizon_rejections,entry.retained_at_10000].every(value=>Number.isInteger(value)&&value>=0) && entry.endpoint_voluntary_rejections + entry.endpoint_administrative_completions + entry.endpoint_censors === entry.independent_environments && entry.retained_at_10000 === entry.endpoint_administrative_completions && entry.support_at_10000 === entry.horizon_rejections + entry.retained_at_10000 && entry.horizon_rejections <= entry.endpoint_voluntary_rejections && entry.endpoint_terminal_competing_causes === 0 && entry.endpoint_unresolved_decisions === 0;
  if (!endpointDisclosure) errors.push("A ranked row requires a passing 0.2-J1 endpoint disclosure with one resolved decision per environment, exact-horizon risk-set accounting, and zero terminal competing causes.");
  ...
  const samplingStoppingDisclosure = entry.sampling_stopping_integrity_verified === true && Number.isInteger(entry.sampling_planned_units) && entry.sampling_planned_units >= 20 && Number.isInteger(entry.sampling_actual_units) && entry.sampling_actual_units === entry.independent_environments && Number.isInteger(entry.sampling_operational_overshoot_units) && entry.sampling_operational_overshoot_units >= 0 && entry.sampling_actual_units === entry.sampling_planned_units + entry.sampling_operational_overshoot_units && numeric(entry.sampling_planned_exposure_hours) && entry.sampling_planned_exposure_hours >= 10000 && numeric(entry.sampling_actual_exposure_hours) && Math.abs(entry.sampling_actual_exposure_hours-entry.total_resident_hours)<.000001 && entry.sampling_actual_exposure_hours >= entry.sampling_planned_exposure_hours && entry.sampling_primary_outcome_access_before_cutoff === 0 && entry.sampling_unscheduled_primary_analyses === 0 && entry.sampling_result_informed_extensions === 0 && entry.sampling_result_informed_early_stops === 0 && entry.sampling_target_changes_after_activity === 0 && entry.sampling_cutoff_changes_after_activity === 0 && entry.sampling_excluded_activated_units === 0 && entry.sampling_excluded_overshoot_units === 0;
  if (!samplingStoppingDisclosure) errors.push("A ranked row requires a passing 0.2-ST1 prospective closure with frozen sufficient targets, exact retained units and exposure, and zero outcome-driven access, changes, or stops.");
...
const samplingStoppingRegistryRequired = ["sampling_stopping_integrity_verified","sampling_planned_units","sampling_actual_units","sampling_operational_overshoot_units","sampling_planned_exposure_hours","sampling_actual_exposure_hours","sampling_primary_outcome_access_before_cutoff","sampling_unscheduled_primary_analyses","sampling_result_informed_extensions","sampling_result_informed_early_stops","sampling_target_changes_after_activity","sampling_cutoff_changes_after_activity","sampling_excluded_activated_units","sampling_excluded_overshoot_units"];
...
const siteHeterogeneityRegistryRequired = ["site_heterogeneity_verified","site_count","smallest_site_n","maximum_site_share","identifiable_site_count","site_wanted_min","site_wanted_max","site_wanted_range","leave_one_site_out_maximum_absolute_shift","site_leave_one_out_unidentifiable"];
export const leaderboardEntrySchema = {
  ...
  properties: {
    ...
    sampling_stopping_integrity_verified:{const:true},sampling_planned_units:{type:"integer",minimum:20},sampling_actual_units:{type:"integer",minimum:20},sampling_operational_overshoot_units:{type:"integer",minimum:0},sampling_planned_exposure_hours:{type:"number",minimum:10000},sampling_actual_exposure_hours:{type:"number",minimum:10000},sampling_primary_outcome_access_before_cutoff:{const:0},sampling_unscheduled_primary_analyses:{const:0},sampling_result_informed_extensions:{const:0},sampling_result_informed_early_stops:{const:0},sampling_target_changes_after_activity:{const:0},sampling_cutoff_changes_after_activity:{const:0},sampling_excluded_activated_units:{const:0},sampling_excluded_overshoot_units:{const:0},
...
leaderboardEntrySchema.required.push(...samplingStoppingRegistryRequired);
leaderboardEntrySchema.required.push(...protocolDeviationRegistryRequired);
leaderboardEntrySchema.required.push(...siteHeterogeneityRegistryRequired);
...
export const leaderboardContract = {
  ...
  ranking: {
    ...
    forbidden_tiebreakers: ["confidence_interval_width", "safety_incidents", "root_envelope_count", "preregistration_amendment_count", "sampling_target_or_overshoot", "protocol_deviation_count", "site_heterogeneity_metrics", "assistance_burden", "participant_labor", "teleoperation", "maintenance", "availability", "service_continuity_metrics", "endpoint_review_metrics", "MTBHR", "human_keep_value_burden_trust", "learning_delta", "generalization_ratio", "policy_update_count", "policy_changed_exposure", "rollout_lag", "privacy_metrics", "reservation_value", "reacquisition", "certification_badges"],
  ...
  admission: ["audit_manifest_passes_all_six_gates", "WANTED_WILD_certification_awarded", "preregistration_integrity_profile_0.2-PR1_passes", "sampling_stopping_integrity_profile_0.2-ST1_passes", "protocol_deviation_integrity_profile_0.2-DV1_passes", "cohort_integrity_profile_0.2-E1_passes", "site_heterogeneity_profile_0.2-SH1_passes", "exposure_ledger_profile_0.2-X1_passes", "root_envelope_profile_0.2-RE1_passes", "root_envelope_batch_profile_0.2-REB1_passes", "root_commitment_witness_profile_0.2-RC1_passes", "endpoint_adjudication_profile_0.2-J1_passes", "analysis_reproduction_profile_0.2-A2_passes", "human_measures_profile_0.2-H1_passes", "learning_generalization_profile_0.2-LG1_passes", "assistance_integrity_profile_0.2-I1_passes", "policy_evolution_integrity_profile_0.2-U1_passes", "privacy_integrity_profile_0.2-PV1_passes", "service_continuity_profile_0.2-SC1_passes", "withdrawal_profile_0.2-W1_when_lifetime_completions_exist", "telemetry_authenticity_profile_0.2-T1_passes", "telemetry_exposure_reconciliation_0.2-TX1_passes", "independent_audit_seal_profile_0.2-V1_passes", "auditor_credential_profile_0.2-V2_passes", "horizon_identifiable", "bootstrap_valid_fraction_at_least_0.95", "preflight_profile_0.2-P1_passes", "safety_profile_0.2-S1_passes", "L4_equals_zero", "public_aggregate_evidence_pack"],
