...
export function assessManifest(text: string): ReadinessResult {
  ...
  const gates = [
    ...
    gate("G4", "PRIMARY + ROBUSTNESS + DIAGNOSTICS", statisticsPass && diagnosticPass, statisticsNeeded ? "W, 95% CI, censoring envelope, influence, tail support, and diagnostics are complete." : "The non-ranking diagnostic profile is complete; a cohort W is not required for this target.", diagnosticPass ? "Provide an identifiable 10K W, enclosing 95% CI, profile 0.2-R1 bounds, influence, tail support, ≥95% valid bootstrap support, and ≥1,000 draws." : "Provide profile 0.2-D1 with valid denominators, intervals, latency tails, and zero-event lower bounds."),
    gate("G5", "SAFETY CASE + TELEMETRY", safetyPass && telemetryPass, "Safety profile 0.2-S1 and signed telemetry pass with complete trials, controls, evidence, and L4=0.", "Provide a passing 0.2-S1 safety case: bound policy and evidence, required stop/remote-loss/rollback trials, full context and privileged-path coverage, no unacceptable residual risk, open material incident, high-severity vulnerability, safety termination, or L4 event; then provide conformant telemetry."),
