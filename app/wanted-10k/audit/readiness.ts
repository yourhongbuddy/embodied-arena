...
export function assessManifest(text: string): ReadinessResult {
  ...
  const submission = object(parsed.submission), organization = object(parsed.organization), robot = object(parsed.robot), study = object(parsed.study), preflight = object(parsed.preflight), cohort = object(parsed.cohort), primary = object(parsed.primary), diagnostics = object(parsed.diagnostics), safety = object(parsed.safety), telemetry = object(parsed.telemetry), adjudication = object(parsed.adjudication), withdrawal = object(parsed.withdrawal), privacy = object(parsed.privacy), audit = object(parsed.audit);
  const target = String(submission.target_certification || "—");
  const errors: string[] = [];
  const requiredRoots = ["protocol_version", "submission_mode", "submission", "organization", "robot", "study", "preflight", "cohort", "primary", "diagnostics", "safety", "telemetry", "adjudication", "withdrawal", "evidence", "privacy", "audit"];
  ...
  const analysisPass = statisticsNeeded ? statisticsPass && diagnosticPass : diagnosticsNeeded ? notApplicable(primary) && diagnosticPass : notApplicable(primary) && notApplicable(diagnostics);
  ...
  const gates = [
    ...
    gate("G4", "TARGET ANALYSIS PROFILE", analysisPass, statisticsNeeded ? "W, 95% CI, censoring envelope, influence, tail support, and diagnostics are complete." : diagnosticsNeeded ? "The non-ranking diagnostic profile is complete and primary W is explicitly not applicable." : "Primary W and field diagnostics are explicitly not applicable before real exposure.", statisticsNeeded ? "Provide an identifiable 10K W, enclosing 95% CI, profile 0.2-R1 bounds, influence, tail support, ≥95% valid bootstrap support, ≥1,000 draws, and profile 0.2-D1 diagnostics." : diagnosticsNeeded ? "Provide profile 0.2-D1 diagnostics and mark primary W not applicable for this non-ranking target." : "Mark both primary W and longitudinal field diagnostics not applicable with reasons."),
    gate("G5", fieldEvidenceNeeded ? "SAFETY CASE + TELEMETRY" : "FIELD EVIDENCE APPLICABILITY", operationsPass, fieldEvidenceNeeded ? "Safety profile 0.2-S1 and signed telemetry pass with complete trials, controls, evidence, and L4=0." : "Field safety and telemetry are correctly marked not applicable for simulation-only prequalification.", fieldEvidenceNeeded ? "Provide a passing 0.2-S1 safety case and conformant telemetry with bound evidence and no hard failure." : "PREQUALIFIED is simulation-only; mark field safety and field telemetry not applicable rather than fabricating field results."),
  ...
  const projection = {
    ...
    rankable: target === "WANTED_WILD", wanted_score: numeric(primary.wanted_score) ? primary.wanted_score : null, ci95: numeric(primary.ci95_lower) && numeric(primary.ci95_upper) ? [primary.ci95_lower, primary.ci95_upper] : null, environments: cohort.independent_environments,
    resident_hours: cohort.total_resident_hours, survival_at_10000: numeric(primary.survival_at_10000) ? primary.survival_at_10000 : null,
