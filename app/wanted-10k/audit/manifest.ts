...
import { assessSiteHeterogeneity, siteHeterogeneityTemplate } from "../site-heterogeneity/profile.ts";
...
const rootEnvelopeIntegritySummary = { type:"object",additionalProperties:false,required:["profile_version","verification_profile","status","target_certification","manifest_uri","manifest_sha256","root_commitments_sha256","deployment_count","root_count","verified_envelopes","verified_signed_prefixes","missing_envelopes","invalid_envelopes","prefix_binding_failures","key_manifest_mismatches","chain_failures","cadence_failures","qualified_assessor","assessor_attested"],properties:{profile_version:{const:"0.2-RE1"},verification_profile:{const:"0.2-REB1"},status:{const:"passed"},target_certification:{enum:["WANTED_LAB","WANTED_WILD","WANTED_10K"]},manifest_uri:uri,manifest_sha256:digest,root_commitments_sha256:digest,deployment_count:{type:"integer",minimum:1},root_count:{type:"integer",minimum:1},verified_envelopes:{type:"integer",minimum:1},verified_signed_prefixes:{type:"integer",minimum:1},missing_envelopes:{const:0},invalid_envelopes:{const:0},prefix_binding_failures:{const:0},key_manifest_mismatches:{const:0},chain_failures:{const:0},cadence_failures:{const:0},qualified_assessor:{type:"string",minLength:1},assessor_attested:{const:true}}};
...
const siteProfileSummary = { type:"object",additionalProperties:false,required:["site_id","environment_count","environment_share","wanted_score","survival_at_10000","horizon_identifiable"],properties:{site_id:{type:"string",pattern:"^site_[a-z0-9_-]{2,40}$"},environment_count:{type:"integer",minimum:3},environment_share:{type:"number",exclusiveMinimum:0,maximum:.5},wanted_score:{type:["number","null"],minimum:0,maximum:100},survival_at_10000:{type:["number","null"],minimum:0,maximum:1},horizon_identifiable:{type:"boolean"}} };
const siteLeaveOneOutSummary = { type:"object",additionalProperties:false,required:["excluded_site_id","remaining_environment_count","wanted_score","shift_from_pooled"],properties:{excluded_site_id:{type:"string",pattern:"^site_[a-z0-9_-]{2,40}$"},remaining_environment_count:{type:"integer",minimum:1},wanted_score:{type:["number","null"],minimum:0,maximum:100},shift_from_pooled:{type:["number","null"],minimum:-100,maximum:100}} };
const siteHeterogeneitySummary = { type:"object",additionalProperties:false,required:["profile_version","status","target_certification","manifest_uri","manifest_sha256","environment_count","site_count","smallest_site_n","maximum_site_share","pooled_wanted_score","pooled_survival_at_10000","identifiable_site_count","site_wanted_min","site_wanted_max","site_wanted_range","leave_one_site_out_maximum_absolute_shift","leave_one_site_out_unidentifiable","site_profiles","leave_one_site_out","cohort_integrity_sha256","endpoint_table_sha256","analysis_reproduction_sha256","preregistration_sha256","qualified_assessor","assessor_attested"],properties:{profile_version:{const:"0.2-SH1"},status:{const:"passed"},target_certification:{const:"WANTED_WILD"},manifest_uri:uri,manifest_sha256:digest,environment_count:{type:"integer",minimum:20},site_count:{type:"integer",minimum:3},smallest_site_n:{type:"integer",minimum:3},maximum_site_share:{type:"number",exclusiveMinimum:0,maximum:.5},pooled_wanted_score:{type:"number",minimum:0,maximum:100},pooled_survival_at_10000:{type:"number",minimum:0,maximum:1},identifiable_site_count:{type:"integer",minimum:0},site_wanted_min:{type:["number","null"],minimum:0,maximum:100},site_wanted_max:{type:["number","null"],minimum:0,maximum:100},site_wanted_range:{type:["number","null"],minimum:0,maximum:100},leave_one_site_out_maximum_absolute_shift:{type:["number","null"],minimum:0,maximum:100},leave_one_site_out_unidentifiable:{type:"integer",minimum:0},site_profiles:{type:"array",minItems:3,items:siteProfileSummary},leave_one_site_out:{type:"array",minItems:3,items:siteLeaveOneOutSummary},cohort_integrity_sha256:digest,endpoint_table_sha256:digest,analysis_reproduction_sha256:digest,preregistration_sha256:digest,qualified_assessor:{type:"string",minLength:1},assessor_attested:{const:true}} };
...
export const auditManifestSchema = {
  ...
  properties: {
    ...
    root_envelope_integrity: { oneOf: [rootEnvelopeIntegritySummary, notApplicable] },
  ...
  allOf: [
    ...
    },
    {
      ...
      then: { properties: { root_envelope_integrity: { not: notApplicable }, root_commitment_witness: { not: notApplicable }, evidence: { allOf: [
        ...
        { contains: { properties: { role: { const: "root_envelope_verification_report" } }, required: ["role"] }, minContains: 1 },
      ] } } },
    },
...
function rootEnvelopeIntegrityAuditSummary(target: "WANTED_LAB" | "WANTED_WILD" | "WANTED_10K") {
  const witness=rootCommitmentWitnessAuditSummary(target);
  return { profile_version:"0.2-RE1",verification_profile:"0.2-REB1",status:"passed",target_certification:target,manifest_uri:"https://example.org/wanted-root-envelope-verification.json",manifest_sha256:hash("96"),root_commitments_sha256:witness.root_commitments_sha256,deployment_count:witness.deployment_count,root_count:witness.root_count,verified_envelopes:witness.root_count,verified_signed_prefixes:witness.root_count,missing_envelopes:0,invalid_envelopes:0,prefix_binding_failures:0,key_manifest_mismatches:0,chain_failures:0,cadence_failures:0,qualified_assessor:"Synthetic Root Envelope Auditor",assessor_attested:true };
}
...
function siteHeterogeneityAuditSummary() {
  const source = structuredClone(siteHeterogeneityTemplate);
  source.bindings.cohort_integrity_sha256 = cohortIntegrityAuditSummary("WANTED_WILD").manifest_sha256;
  source.bindings.endpoint_table_sha256 = analysisReproductionTemplate.evidence.endpoint_table_sha256;
  source.bindings.analysis_reproduction_sha256 = hash("aa");
  source.bindings.preregistration_sha256 = hash("b");
  const assessment = assessSiteHeterogeneity(source);
  if (!assessment.summary || assessment.status !== "passed") throw new Error("Synthetic WANTED_WILD site-heterogeneity profile must pass.");
  return { ...assessment.summary, profile_version: "0.2-SH1", status: "passed", target_certification: "WANTED_WILD", manifest_uri: source.evidence.reproduction_log_uri, manifest_sha256: source.evidence.reproduction_log_sha256, ...source.bindings, qualified_assessor: source.assessor.name, assessor_attested: source.assessor.attested };
}
...
const siteHeterogeneitySummaryValue = siteHeterogeneityAuditSummary();
...
const auditManifestTemplateBase = {
  ...
  root_envelope_integrity: rootEnvelopeIntegrityAuditSummary("WANTED_WILD"),
  ...
  evidence: [
    ...
    { role: "root_envelope_verification_report", uri: "https://example.org/wanted-root-envelope-verification.json", sha256: hash("96"), public: true },
    ...
    { role: "telemetry_exposure_reconciliation", uri: "https://example.org/wanted-telemetry-exposure-reconciliation.json", sha256: hash("9c"), public: true },
  ...
  audit: { profile_version: "0.2-V1", auditor: "Synthetic Auditor", auditor_organization: "Independent Example Assurance", independence_statement: "Synthetic example: auditor is organizationally and financially independent of the sponsor.", scope: ["Preregistration freeze and amendment integrity", "Prospective sampling and stopping integrity", "Protocol deviation reconciliation", "Telemetry authenticity, signed root-envelope integrity, exposure reconciliation, and independent root witnessing", "Endpoint dispositions", "Safety evidence", "Score reproduction", "Site heterogeneity and dominance", "Withdrawal integrity", "Human measures integrity", "Learning and generalization integrity", "Assistance integrity", "Policy evolution integrity", "Privacy and consent integrity", "Service continuity integrity"], signed_at: "2026-08-28T18:00:00Z", signature_algorithm: "Ed25519", canonicalization: "RFC8785_JCS", signature_scope: "audit_manifest_without_audit.manifest_sha256_and_audit.auditor_signature", public_key_uri: "https://embodied-arena.chrishongap.chatgpt.site/wanted-10k/auditor-credential.template.json", public_key_base64url: "11qYAYKxCrfVS_7TyWQHOg7hcvPapiMlrwIaaPcHURo", public_key_sha256: "21fe31dfa154a261626bf854046fd2271b7bed4b6abe45aa58877ef47f9721b9", credential: auditorCredentialTemplate, manifest_sha256: hash("a1"), auditor_signature: "A".repeat(86) },
};

const na = (reason: string) => ({ applicable: false as const, reason });
const syntheticAuditSeals = {
  PREQUALIFIED: { manifest_sha256: "8b3093375b674ee2aab34c3ac14132ec9be56ae82465b7d22e8eef9dc1239537", auditor_signature: "u-lltzIX7OiigbQSgCUi6XLQE1HwkDR_OMgerJHrJhwYwFfRHVxmj1WaOtRu28CBn8lFsDVGUcTHwYoEMUTgBw" },
  WANTED_LAB: { manifest_sha256: "f9c5390bfc0eed0131c19de7590570b64ada75dee0e1484ca5dff04e26f14c10", auditor_signature: "_r_m1KcW4iR6h6s7TN6Oym09oA7obiYVCPlFwhGgD--R4FHvzxebzVFhEvFThxl7U4kOim-ollTQDwYJ0a65Bw" },
  WANTED_WILD: { manifest_sha256: "f6e43e262b935bae97a8e3e70248169f244b0a8adb9121e60e18a3ef006ea14f", auditor_signature: "Icj4dSIBZlxOwj3HTHNUOhxnVGlLes68tV360D-d90A83uchC_Mc02ZmOh8GHcqa2h3CHXtU4Lp6Wi2sJa4XDg" },
  WANTED_10K: { manifest_sha256: "8d5d0cac9752b983bbb2f0914326b35bbcb1dba854dcaa1fa9cbc2e7733189b8", auditor_signature: "AnXVN5dVBkCr0OOVK1yM9fsIL4uvSndfKpcgkG1iJQeNF1POb2gS_tOs_naH6cvq-k4Ffy2Qj4iK9RfHGXCPCw" },
};
...
export function auditManifestTemplateFor(target: "PREQUALIFIED" | "WANTED_LAB" | "WANTED_WILD" | "WANTED_10K") {
  ...
  if (target === "PREQUALIFIED") {
    ...
    value.root_envelope_integrity = na("Signed telemetry root envelopes begin only with real field deployments.");
  ...
  if (target === "WANTED_LAB") {
    ...
    value.root_envelope_integrity = rootEnvelopeIntegrityAuditSummary("WANTED_LAB");
    ...
    value.evidence = value.evidence.filter((item: { role: string }) => !["withdrawal_results", "site_heterogeneity_report"].includes(item.role));
  ...
  if (target === "WANTED_10K") {
    ...
    value.root_envelope_integrity = rootEnvelopeIntegrityAuditSummary("WANTED_10K");
    ...
    value.evidence = value.evidence.filter((item: { role: string }) => item.role !== "site_heterogeneity_report");
  }
