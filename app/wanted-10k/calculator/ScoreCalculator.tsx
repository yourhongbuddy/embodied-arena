...
import { bootstrap, HORIZON, score, type Outcome, type Row } from "./scoring";
...
export function ScoreCalculator() {
  ...
  const rankable = rows.length >= 20 && totalHours >= 10_000 && result.identifiable && interval !== null && terminalCauses === 0;
  const statisticalStatus = !result.identifiable ? "HORIZON UNSUPPORTED" : interval === null ? "CI UNSTABLE" : terminalCauses ? "TERMINAL REVIEW" : rankable ? "THRESHOLD MET" : "PROVISIONAL";

  const update = (id: number, patch: Partial<Row>) => setRows(current => current.map(row => row.id === id ? { ...row, ...patch } : row));
  const add = () => setRows(current => [...current, { id: Math.max(0,...current.map(row=>row.id)) + 1, environment: `ENV-${String(current.length + 1).padStart(3,"0")}`, hours: 0, outcome: "unrelated_censor" }]);
  ...
  return <section className="calcWorkspace shell">
    ...
    <div className="calcGrid">
      <div className="cohortEditor">
        ...
        <div className="cohortRows">{rows.map(row=><div className="cohortRow" key={row.id}>
          ...
          <label><span>Outcome</span><select aria-label={`Outcome for ${row.environment}`} value={row.outcome} onChange={event=>update(row.id,{outcome:event.target.value as Outcome})}><option value="completed">Completed at 10K</option><option value="rejected">Voluntary rejection</option><option value="unrelated_censor">Unrelated exit / censor</option><option value="safety_termination">Safety termination</option><option value="developer_withdrawal">Developer withdrawal</option><option value="consent_privacy_withdrawal">Consent / privacy withdrawal</option></select></label>
    ...
    <div className="calcCaution"><b>{result.identifiable ? "STATISTICAL ELIGIBILITY IS NOT CERTIFICATION." : "10K HORIZON IS NOT IDENTIFIABLE."}</b><span>{result.identifiable ? "A qualifying cohort still needs complete intervention disclosure, passed safety gates, preregistration, and independent audit." : "At least one observation must support 10,000 hours, or the Kaplan–Meier curve must reach zero before support ends. WANTED never extends the last observed survival level to manufacture unobserved hours."}</span></div>
  </section>;
}
