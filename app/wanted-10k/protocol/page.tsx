...
const kit = [
  ...
  ["SITE HETEROGENEITY LAB", "Verify that a ranked WANTED WILD cohort spans at least three substantive sites, no site dominates, and pooled conclusions survive complete site exclusions.", "/wanted-10k/site-heterogeneity", "OPEN SITE LAB →"],
  ["SITE HETEROGENEITY CONTRACT", "Read the 0.2-SH1 site topology, unchanged pooled estimator, site-specific and leave-one-site-out disclosures, privacy boundary, and non-ranking rule.", "/wanted-10k/site-heterogeneity.json", "OPEN CONTRACT ↗"],
  ["SITE HETEROGENEITY SCHEMA", "Validate opaque site assignments, exact pooled and site-level claims, complete site exclusions, and bound upstream evidence.", "/wanted-10k/site-heterogeneity.schema.json", "OPEN SCHEMA ↗"],
  ["SITE HETEROGENEITY TEMPLATE", "Start from a passing synthetic three-site, 24-environment WANTED WILD sensitivity manifest.", "/wanted-10k/site-heterogeneity.template.json", "DOWNLOAD JSON ↓"],
  ...
  ["ROOT COMMITMENT WITNESS", "Prove every periodic telemetry root was disclosed on time to two organizationally independent Ed25519 witnesses.", "/wanted-10k/root-commitment-witness", "OPEN WITNESS PROFILE →"],
  ...
  ["SIGNED ROOT ENVELOPES", "Define exactly what each witnessed digest commits to—verified signed telemetry, frozen keys, boundary, counts, tail, and prior root—and require that proof for every field certification.", "/wanted-10k/root-commitment-witness/envelopes", "OPEN ROOT ENVELOPE GUIDE →"],
  ["ROOT ENVELOPE PROFILE", "Read the 0.2-RE1 canonical digest scope, chain rules, zero-event interval semantics, bindings, and interpretation boundary.", "/wanted-10k/root-envelope.json", "OPEN ENVELOPE PROFILE ↗"],
  ["ROOT ENVELOPE HELPER", "Create or verify one root or a complete cadence series locally using the signed telemetry verifier as its only sibling module.", "/wanted-10k/wanted-root-envelope.mjs", "DOWNLOAD ROOT HELPER ↓"],
  ["ROOT ENVELOPE TEMPLATE", "Download a complete two-root synthetic series, including a zero-event interval and exact RC1 witness projection.", "/wanted-10k/root-envelope.template.json", "DOWNLOAD ROOT SERIES ↓"],
  ...
  ["ROOT WITNESS CONFORMANCE", "Exercise 11 normative passing and adversarial manifests covering cadence, tamper, replay, key lifecycle, quorum, and assessor chronology.", "/wanted-10k/root-witness-conformance-vectors.json", "DOWNLOAD VECTORS ↓"],
  ["ROOT WITNESS SDK CONTRACT", "Pin verifier version 0.2-RCS2, exports, source and vector-pack digests, runtime requirements, privacy boundary, and CLI exit codes.", "/wanted-10k/root-witness-verifier-sdk.json", "OPEN SDK CONTRACT ↗"],
  ["ROOT ENVELOPE BATCH SCHEMA", "Validate the local-only 0.2-REB1 cross-deployment input used to reproduce audit counters and the exact RC1 root collection.", "/wanted-10k/root-envelope-batch.schema.json", "OPEN BATCH SCHEMA ↗"],
  ["ROOT ENVELOPE CONFORMANCE", "Run all 10 normative 0.2-REBC1 vectors and match status, failed topology, and every aggregate counter exactly.", "/wanted-10k/root-envelope-conformance-vectors.json", "DOWNLOAD CONFORMANCE PACK ↓"],
  ["ROOT ENVELOPE SDK CONTRACT", "Pin helper version 0.4-RES3, source and vector-pack digests, single, series, batch, and conformance CLI modes, bindings, and privacy boundary.", "/wanted-10k/root-envelope-sdk.json", "OPEN ENVELOPE SDK ↗"],
  ["WITNESS RECEIPT ISSUANCE", "Prepare, externally sign, attach, and self-verify one RC1 receipt while production keys remain in witness-owned secure infrastructure.", "/wanted-10k/root-commitment-witness/issuance", "OPEN ISSUANCE GUIDE →"],
  ["WITNESS RECEIPT HELPER", "Download the zero-dependency 0.2-RIS1 module and three-mode Node CLI; no private-key input or network access exists.", "/wanted-10k/wanted-root-witness-receipt.mjs", "DOWNLOAD HELPER ↓"],
  ["WITNESS RECEIPT CONTRACT", "Pin source digest, HSM callback boundary, safe CLI modes, exact receipt fields, lifecycle checks, and interpretation limits.", "/wanted-10k/root-witness-receipt-sdk.json", "OPEN RECEIPT CONTRACT ↗"],
  ["WITNESS RECEIPT SCHEMA", "Validate unsigned receipts, signed receipts, and witness registry keys at the independent-operator boundary.", "/wanted-10k/root-witness-receipt.schema.json", "OPEN RECEIPT SCHEMA ↗"],
  ...
  ["ANALYSIS CONFORMANCE VECTORS", "Run nine normative 0.2-AC4 cases with exact outputs for the A2 estimator, tie rule, horizon boundary, terminal causes, canonical independent units, and full-horizon completion.", "/wanted-10k/analysis-conformance-vectors.json", "DOWNLOAD VECTORS ↓"],
  ...
  ["ANALYSIS CONFORMANCE RUNNER", "Execute the entire 0.2-AC4 pack with a zero-dependency ESM module and receive field-level mismatch reports.", "/wanted-10k/wanted-analysis-conformance.mjs", "DOWNLOAD RUNNER ↓"],
  ["ANALYSIS RUNNER CONTRACT", "Pin runner version 0.2-ACS4, exported functions, source digest, vector-pack digest, and the non-certification boundary.", "/wanted-10k/analysis-conformance-sdk.json", "OPEN CONTRACT ↗"],
  ...
  ["SAMPLING + STOPPING LAB", "Verify that the sample target, exposure floor, cutoff, outcome-access boundary, monitoring, and overshoot handling were fixed before results.", "/wanted-10k/sampling-stopping", "OPEN CLOSURE LAB →"],
  ["SAMPLING + STOPPING CONTRACT", "Read the 0.2-ST1 prospective closure, outcome-blind monitoring, no-adaptation, overshoot-retention, and interpretation rules.", "/wanted-10k/sampling-stopping.json", "OPEN CONTRACT ↗"],
  ["SAMPLING + STOPPING SCHEMA", "Validate frozen target minima, exact calendar closure, zero early primary access, and every result-independent execution claim.", "/wanted-10k/sampling-stopping.schema.json", "OPEN SCHEMA ↗"],
  ["SAMPLING + STOPPING TEMPLATE", "Start from a passing target-specific synthetic plan and closure history with independent blinded monitoring.", "/wanted-10k/sampling-stopping.template.json", "DOWNLOAD JSON ↓"],
...
export default function ProtocolPage() {
  return <main className="protocolPage">
    ...
    <section className="kitSection" id="resources"><div className="shell">
      <div className="sectionHead wantedHead"><div><span className="kicker">03 / DEVELOPER + STUDY KIT</span><h2>One hundred six artifacts.<br/><em>One evidence chain.</em></h2></div><p>Everything needed to choose a certification claim, freeze and verify preregistration, prove result-independent sampling and closure, reconcile every protocol deviation, preflight a policy, prove cohort and site heterogeneity, clock, telemetry, endpoint adjudication, privacy, service continuity, final-manifest authenticity, registered auditor-key trust, neutral human reports, matched learning, complete assistance burden, immutable policy evolution, realtime closed-loop burden, binding preference choices, and a seven-day absence, verify field safety, connect a robot, reproduce the score and diagnostics, and publish an immutable registry row.</p></div>
