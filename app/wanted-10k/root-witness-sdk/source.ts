export const ROOT_WITNESS_VERIFIER_SDK_VERSION="0.2-RCS1";

export const rootWitnessVerifierSdkSource=String.raw`/**
 * WANTED-10K independent root-witness verifier — 0.2-RCS1 / profile 0.2-RC1
...
export async function runCli(args=[],io={}){const stdout=typeof io.stdout==="function"?io.stdout:value=>console.log(value),stderr=typeof io.stderr==="function"?io.stderr:value=>console.error(value),usage="Usage: node wanted-root-witness-verifier.mjs ROOT-WITNESS-MANIFEST.json";if(args.length===1&&["--help","-h"].includes(args[0])){stdout(usage);return CLI_EXIT_CODES.pass}if(args.length!==1){stderr(usage);return CLI_EXIT_CODES.usage_or_io_error}try{const result=await verifyRootWitnessFile(args[0]);stdout(JSON.stringify(result,null,2));return result.status==="passed"?CLI_EXIT_CODES.pass:CLI_EXIT_CODES.verification_failed}catch(error){stderr("WANTED root-witness verifier: "+(error instanceof Error?error.message:"unable to read or verify input"));return CLI_EXIT_CODES.usage_or_io_error}}
...
export const rootWitnessVerifierSdkContract={name:"WANTED Root Witness Verifier SDK",version:ROOT_WITNESS_VERIFIER_SDK_VERSION,protocol_version:"0.2",profile:"0.2-RC1",module:"/wanted-10k/wanted-root-witness-verifier.mjs",format:"JavaScript ESM",runtime_dependencies:0,runtime_requirements:["Web Crypto Ed25519","TextEncoder","structuredClone","atob"],performs_network_requests:false,input:"0.2-RC1 root-witness manifest JSON",exports:["canonicalize","unsignedWitnessReceipt","witnessReceiptSigningBytes","canonicalRootCollection","rootCollectionSha256","verifyRootWitnessManifest","verifyRootWitnessFile","CLI_EXIT_CODES","runCli"],cli:{runtime:"Node.js 22+",usage:"node wanted-root-witness-verifier.mjs ROOT-WITNESS-MANIFEST.json",stdout:"JSON verification report",stderr:"usage or input error",exit_codes:{pass:0,verification_failed:1,usage_or_io_error:2}},verifies:["strict_I-JSON","frozen_protocol","witness_registry_independence","complete_commitment_cadence","canonical_root_collection_digest","receipt_identity_and_log_uniqueness","bounded_publication_delay","witness_key_lifecycle","every_Ed25519_signature","two_organization_quorum","target_coverage","assessor_chronology"],result:"pass_only_when_all_eight_RC1_gates_pass",privacy:"local_only_no_manifest_uploads_or_network_requests",interpretation:"proves timely independent disclosure of the root collection, not sensor truth, complete event capture, safety, human preference, or certification"} as const;
