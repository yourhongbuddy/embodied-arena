...
const number = (value: number) => new Intl.NumberFormat("en-US", { maximumFractionDigits: 3, notation: Math.abs(value) >= 1e6 ? "compact" : "standard" }).format(value);
...
export function renderBenchmarkSvg(input: BenchmarkDocument): string {
  ...
  const title = escapeXml(doc.title), displayTitle = escapeXml(doc.title.length > Math.floor((width - left - 40) / 14) ? doc.title.slice(0, Math.floor((width - left - 40) / 14) - 1) + "…" : doc.title), label = escapeXml(`${metric.name}${metric.unit ? ` (${metric.unit})` : ""}`);
  ...
  if (scatter && xMetric) {
    ...
    doc.rows.forEach((row, index) => {
      ...
      const displayLabel = row.label.length > 24 ? `${row.label.slice(0, 22)}…` : row.label;
