...
import { experimentResultsSnapshot } from "../../experiments/snapshot.ts";
...
export async function readExperimentResults(db:D1Binding){
  ...
  const payload={status:"ready",experiment:WANTED_LANDING_EXPERIMENT.id,analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,analysis_window_status:"bound",analysis_window:analysisWindow,analysis_snapshot_status:"bound",assignment_receipt_profile:EXPERIMENT_ASSIGNMENT_RECEIPT_PROFILE,assignment_receipt_required:true,receipt_integrity:summarizeReceiptIntegrity(receiptIntegrity.results),implementation_version:ROTATOR_VERSION,analysis_unit:"experiment_scoped_anonymous_browser_unit",unit_represents:"one_first_party_browser_profile_storage_instance",reported_as_unique_users:false,human_identity_resolution:false,window_days:EXPERIMENT_REPORTING_WINDOW_DAYS,primary_goal:WANTED_LANDING_EXPERIMENT.primary_goal,cross_variant_units_excluded:Number(integrity?.cross_variant_units||0),multi_token_units_excluded:Number(integrity?.multi_token_units||0),...summarizeExperiment(result.results)} as const;
  return{...payload,analysis_snapshot:await experimentResultsSnapshot(payload)} as const;
}

export function experimentResultsResponse(result:Awaited<ReturnType<typeof readExperimentResults>>){return Response.json(result,{headers:{"cache-control":"no-store","x-experiment-snapshot-digest":result.analysis_snapshot.digest}})}

export async function GET() {
  try{return experimentResultsResponse(await readExperimentResults(await getD1()))}
  catch{return Response.json({status:"unavailable",experiment:WANTED_LANDING_EXPERIMENT.id,analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,analysis_window_status:"unavailable",analysis_window:null,analysis_snapshot_status:"unavailable",analysis_snapshot:null,assignment_receipt_profile:EXPERIMENT_ASSIGNMENT_RECEIPT_PROFILE,assignment_receipt_required:true,receipt_integrity:summarizeReceiptIntegrity(null),implementation_version:ROTATOR_VERSION,analysis_unit:"experiment_scoped_anonymous_browser_unit",unit_represents:"one_first_party_browser_profile_storage_instance",reported_as_unique_users:false,human_identity_resolution:false,window_days:EXPERIMENT_REPORTING_WINDOW_DAYS,primary_goal:WANTED_LANDING_EXPERIMENT.primary_goal,cross_variant_units_excluded:0,multi_token_units_excluded:0,...emptySummary()},{headers:{"cache-control":"no-store"}})}
}
