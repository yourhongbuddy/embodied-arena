...
test("requires the quantitative safety case for audit readiness",()=>{const passing=assessManifest(JSON.stringify(auditManifestTemplate));assert.equal(passing.status,"test");assert.equal(passing.gates.find(g=>g.id==="G5").status,"pass");const failed=structuredClone(auditManifestTemplate);failed.safety.context_coverage=.99;const result=assessManifest(JSON.stringify(failed));assert.equal(result.status,"not_ready");assert.equal(result.gates.find(g=>g.id==="G5").status,"fail");const unbound=structuredClone(auditManifestTemplate);unbound.safety.policy_artifact_sha256=`${"e".repeat(63)}d`;assert.equal(assessManifest(JSON.stringify(unbound)).gates.find(g=>g.id==="G5").status,"fail")});
