...
const entry = (submission_id, public_label, wanted_score, overrides = {}) => ({
  ...
  root_envelope_integrity_verified:true, root_envelope_count:1000,
...
test("requires root-envelope integrity but never uses envelope count to break W ties",()=>{const malformed=entry("bad-envelope","Bad envelope",75,{root_envelope_integrity_verified:false});let result=rankRegistry([malformed]);assert.equal(result.ranked.length,0);assert.match(result.excluded[0].reasons.join(" "),/0.2-RE1/);const tie=rankRegistry([entry("few-envelopes","Alpha",80.01,{root_envelope_count:24}),entry("many-envelopes","Beta",80.04,{root_envelope_count:5000})]);assert.deepEqual(tie.ranked.map(row=>[row.public_label,row.rank]),[["Alpha",1],["Beta",1]])});
