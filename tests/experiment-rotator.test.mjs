...
test("freezes a privacy-first presentation-only boundary",()=>{
  ...
  assert.deepEqual(experimentRotatorContract.analysis_cohort.legacy_versions_included,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32"]);
  assert.deepEqual(experimentRotatorContract.analysis_cohort.active_event_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33"]);
  assert.deepEqual(EXPERIMENT_ACTIVE_COHORT_EVENT_VERSIONS,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33"]);
  ...
  assert.deepEqual(experimentRotatorContract.ingestion.accepted_rotator_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33"]);
...
test("accepts only active-cohort, internal, schema-valid experiment events",()=>{
  ...
  const exposure={experiment:"wanted_landing_v1",analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,assignment_receipt:"66666666-6666-4666-8666-666666666666",unit_id:unit,variant:"control",assignment_mode:"assigned",rotator_version:"0.33-R33",exposure_id:token};
  ...
  assert.equal(validActiveCohortEventVersion("0.33-R33"),true);assert.equal(validActiveCohortEventVersion("0.32-R32"),true);assert.equal(validActiveCohortEventVersion("0.31-R31"),true);assert.equal(validActiveCohortEventVersion("0.30-R30"),true);assert.equal(validActiveCohortEventVersion("0.29-R29"),true);assert.equal(validActiveCohortEventVersion("0.28-R28"),true);assert.equal(validActiveCohortEventVersion("0.27-R27"),false);assert.equal(validActiveCohortEventVersion("0.34-R34"),false);
  ...
  assert.equal(validExperimentEvent("experiment_exposure","/wanted-10k",{...exposure,rotator_version:"0.34-R34"}),false);
