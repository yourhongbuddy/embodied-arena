...
test("freezes a privacy-first presentation-only boundary",()=>{
  ...
  assert.equal(experimentRotatorContract.receipt_diagnostics.profile,"0.24-RD3");
...
test("executes the matched-exposure query against SQLite",()=>{
  ...
  const compatibleReleaseUnit=unitForVariant("control",2);event({session:"compatible_release",eventType:"experiment_exposure",variant:"control",unit:compatibleReleaseUnit,version:"0.25-R25"});event({session:"compatible_release",eventType:"experiment_goal",variant:"control",unit:compatibleReleaseUnit,version:"0.25-R25",goal:"primary_cta"});
...
test("reports the indexed receipt-to-exposure funnel without inventing rejected traffic",async()=>{
  ...
  assert.equal(balanced.issuance_sample_ratio_mismatch.status,"pass");assert.equal(balanced.unit_delivery_balance.status,"pass");
  ...
  assert.equal(summarizeReceiptIntegrity(null).receipt_to_exposure_rate,null);assert.equal(summarizeReceiptIntegrity([{variant:"control",issued_receipts:1,issued_units:1,exposed_receipts:4,exposed_units:4,matured_units:4,matured_exposed_units:4}]).exposed_receipts,1);db.close();
});
...
test("accepts only current, internal, schema-valid experiment events",()=>{
  ...
  const exposure={experiment:"wanted_landing_v1",analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,assignment_receipt:"66666666-6666-4666-8666-666666666666",unit_id:unit,variant:"control",assignment_mode:"assigned",rotator_version:"0.24-R24",exposure_id:token};
