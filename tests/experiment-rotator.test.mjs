...
test("freezes a privacy-first presentation-only boundary",()=>{
  ...
  assert.equal(experimentRotatorContract.receipt_diagnostics.profile,"0.23-RD2");
...
test("executes the matched-exposure query against SQLite",()=>{
  ...
  const compatibleReleaseUnit=unitForVariant("control",2);event({session:"compatible_release",eventType:"experiment_exposure",variant:"control",unit:compatibleReleaseUnit,version:"0.24-R24"});event({session:"compatible_release",eventType:"experiment_goal",variant:"control",unit:compatibleReleaseUnit,version:"0.24-R24",goal:"primary_cta"});
...
test("reports the indexed receipt-to-exposure funnel without inventing rejected traffic",async()=>{
  ...
  assert.deepEqual(byVariant.get("control"),{variant:"control",issued_receipts:1,issued_units:1,exposed_receipts:1,exposed_units:1,unexposed_receipts:0,expired_unexposed_receipts:0,duplicate_session_unit_receipts:0});
  assert.deepEqual(byVariant.get("proof"),{variant:"proof",issued_receipts:2,issued_units:1,exposed_receipts:0,exposed_units:0,unexposed_receipts:2,expired_unexposed_receipts:0,duplicate_session_unit_receipts:1});
  assert.deepEqual(byVariant.get("developer"),{variant:"developer",issued_receipts:1,issued_units:1,exposed_receipts:0,exposed_units:0,unexposed_receipts:1,expired_unexposed_receipts:1,duplicate_session_unit_receipts:0});
  assert.equal(summary.profile,"0.23-RD2");assert.equal(summary.issued_receipts,4);assert.equal(summary.issued_units,3);assert.equal(summary.exposed_receipts,1);assert.equal(summary.exposed_units,1);assert.equal(summary.unexposed_receipts,3);assert.equal(summary.active_unexposed_receipts,2);assert.equal(summary.expired_unexposed_receipts,1);assert.equal(summary.duplicate_session_unit_receipts,1);assert.equal(summary.receipt_to_exposure_rate,.25);assert.equal(summary.unit_to_exposure_rate,1/3);assert.equal(summary.variants.length,3);assert.equal(summary.issuance_sample_ratio_mismatch.status,"insufficient");assert.equal(summary.unit_delivery_balance.status,"insufficient");assert.equal(summary.counts_rejected_requests,false);assert.equal(summary.proves_human_traffic,false);
  ...
  const balanced=summarizeReceiptIntegrity(WANTED_LANDING_EXPERIMENT.variants.map(variant=>({variant:variant.id,issued_receipts:100,issued_units:100,exposed_receipts:80,exposed_units:80,unexposed_receipts:20,expired_unexposed_receipts:10,duplicate_session_unit_receipts:0})));
  assert.equal(balanced.issuance_sample_ratio_mismatch.status,"pass");assert.equal(balanced.unit_delivery_balance.status,"pass");
  const drifted=summarizeReceiptIntegrity(WANTED_LANDING_EXPERIMENT.variants.map((variant,index)=>({variant:variant.id,issued_receipts:100,issued_units:100,exposed_receipts:index?50:99,exposed_units:index?50:99,unexposed_receipts:index?50:1,expired_unexposed_receipts:0,duplicate_session_unit_receipts:0})));
  assert.equal(drifted.unit_delivery_balance.status,"alert");assert.equal(drifted.unit_delivery_balance.p_value<.001,true);
  assert.equal(summarizeReceiptIntegrity(null).receipt_to_exposure_rate,null);assert.equal(summarizeReceiptIntegrity([{variant:"control",issued_receipts:1,issued_units:1,exposed_receipts:4,exposed_units:4}]).exposed_receipts,1);db.close();
});
...
test("accepts only current, internal, schema-valid experiment events",()=>{
  ...
  const exposure={experiment:"wanted_landing_v1",analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,assignment_receipt:"66666666-6666-4666-8666-666666666666",unit_id:unit,variant:"control",assignment_mode:"assigned",rotator_version:"0.23-R23",exposure_id:token};
