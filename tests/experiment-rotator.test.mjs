...
test("freezes one exact inclusive settled D1 analysis window",()=>{
  const window=experimentAnalysisWindow("2026-09-01T12:34:56.789Z");
  assert.deepEqual(window,{profile:"0.28-AW2",clock:"d1_database_utc",source:"d1_clock_query_minus_settling_lag",settling_lag_seconds:1,precision:"whole_seconds",started_at:"2026-08-02T12:34:56.000Z",ended_at:"2026-09-01T12:34:56.000Z",start_inclusive:true,end_inclusive:true,duration_days:30});
  assert.equal(Date.parse(window.ended_at)-Date.parse(window.started_at),30*86_400_000);
  assert.throws(()=>experimentAnalysisWindow("not-a-time"),/valid analysis timestamp/);
  assert.match(experimentAnalysisClockQuery,/strftime\('%Y-%m-%dT%H:%M:%SZ','now','-1 second'\)/);
  const clockDb=new DatabaseSync(":memory:"),clock=clockDb.prepare(experimentAnalysisClockQuery).get();
...
test("binds each ready result to one reproducible canonical content snapshot",async()=>{
  ...
  const sealed={...payload,analysis_snapshot:snapshot};assert.equal(await verifyExperimentResultsSnapshot(sealed),true);assert.equal(await verifyExperimentResultsSnapshot({...sealed,z:3}),false);assert.equal(await verifyExperimentResultsSnapshot(payload),false);assert.equal(await verifyExperimentResultsSnapshot({...sealed,analysis_snapshot:{...snapshot,note:"invented"}}),false);
  await assert.rejects(()=>experimentResultsSnapshot({value:Number.NaN}),/non-finite/);const cycle={};cycle.self=cycle;await assert.rejects(()=>experimentResultsSnapshot(cycle),/cycle/);
});
...
test("freezes a privacy-first presentation-only boundary",()=>{
  ...
  assert.deepEqual(experimentRotatorContract.analysis_cohort.legacy_versions_included,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34","0.35-R35"]);
  assert.deepEqual(experimentRotatorContract.analysis_cohort.active_event_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34","0.35-R35","0.36-R36"]);
  assert.deepEqual(EXPERIMENT_ACTIVE_COHORT_EVENT_VERSIONS,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34","0.35-R35","0.36-R36"]);
  ...
  assert.equal(EXPERIMENT_REPORTING_WINDOW_DAYS,30);
  assert.equal(EXPERIMENT_ANALYSIS_SETTLING_LAG_SECONDS,1);
  assert.equal(experimentRotatorContract.counting.reporting_window_days,EXPERIMENT_REPORTING_WINDOW_DAYS);
  assert.equal(experimentRotatorContract.counting.reporting_window.profile,"0.28-AW2");
  assert.equal(experimentRotatorContract.counting.reporting_window.clock,"d1_database_utc");
  assert.equal(experimentRotatorContract.counting.reporting_window.timestamp_source,"d1_clock_query_minus_settling_lag");
  assert.equal(experimentRotatorContract.counting.reporting_window.timestamp_precision,"whole_seconds_matching_d1_current_timestamp");
  assert.equal(experimentRotatorContract.counting.reporting_window.settling_lag_seconds,EXPERIMENT_ANALYSIS_SETTLING_LAG_SECONDS);
  assert.equal(experimentRotatorContract.counting.reporting_window.start_inclusive,true);
  assert.equal(experimentRotatorContract.counting.reporting_window.end_inclusive,true);
  assert.equal(experimentRotatorContract.counting.reporting_window.unavailable_value,null);
  assert.equal(experimentRotatorContract.counting.reporting_window.all_event_queries_share_boundaries,true);
  assert.equal(experimentRotatorContract.counting.reporting_window.event_queries_use_sqlite_now_functions,false);
  assert.equal(experimentRotatorContract.counting.reporting_window.clock_query_uses_sqlite_now,true);
  ...
  assert.equal(experimentRotatorContract.delivery.compatible_revision_delivery_within_cohort,true);
  assert.equal(experimentRotatorContract.delivery.compatibility_grace_bound_by_assignment_receipt_expiry,true);
  ...
  assert.deepEqual(experimentRotatorContract.ingestion.accepted_rotator_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34","0.35-R35","0.36-R36"]);
  ...
  assert.equal(experimentRotatorContract.assignment_sdk.version,"0.32-RSDK1");assert.equal(experimentRotatorContract.assignment_sdk.module,"/experiments/wanted-rotator.mjs");assert.equal(experimentRotatorContract.assignment_sdk.contract,"/experiments/rotator-sdk.json");assert.equal(experimentRotatorContract.assignment_sdk.lab,"/experiments/assignment-lab");assert.equal(experimentRotatorContract.assignment_sdk.lab_contract,"/experiments/assignment-lab.json");assert.equal(experimentRotatorContract.assignment_sdk.conformance_vectors.length,3);assert.equal(experimentRotatorContract.assignment_sdk.changes_assignment,false);assert.equal(experimentRotatorContract.assignment_sdk.issues_server_receipt,false);assert.equal(experimentRotatorContract.assignment_sdk.authenticates_traffic,false);assert.equal(experimentRotatorContract.assignment_sdk.lab_uses_synthetic_units_only,true);assert.equal(experimentRotatorContract.assignment_sdk.lab_sends_general_analytics,false);assert.equal(experimentRotatorContract.assignment_sdk.lab_sends_experiment_analytics,false);assert.equal(experimentRotatorContract.assignment_sdk.lab_flushes_goal_outbox,false);assert.equal(experimentRotatorContract.assignment_sdk.lab_counts_exposures,false);assert.equal(experimentRotatorContract.assignment_sdk.lab_supports_version_selection,false);
  assert.equal(experimentRotatorContract.decision_design_lab.profile,"0.34-DL1");assert.equal(experimentRotatorContract.decision_design_lab.route,"/experiments/design-lab");assert.equal(experimentRotatorContract.decision_design_lab.contract,"/experiments/design-lab.json");assert.equal(experimentRotatorContract.decision_design_lab.plan_schema,"/experiments/design-plan.schema.json");assert.equal(experimentRotatorContract.decision_design_lab.reference_plan,"/experiments/design-plan.reference.json");assert.deepEqual(experimentRotatorContract.decision_design_lab.verifier,{profile:"0.35-DPV1",module:"/experiments/wanted-design-plan.mjs",contract:"/experiments/design-plan-verifier.json",runtime_dependencies:0,performs_network_requests:false});assert.deepEqual(experimentRotatorContract.decision_design_lab.registration_verifier,{profile:"0.36-DPR1",lab:"/experiments/registration-lab",module:"/experiments/wanted-design-registration.mjs",contract:"/experiments/design-registration.json",schema:"/experiments/design-registration.schema.json",reference_bundle:"/experiments/design-registration.reference.json",runtime_dependencies:0,performs_network_requests:false});assert.equal(experimentRotatorContract.decision_design_lab.changes_live_allocation,false);assert.equal(experimentRotatorContract.decision_design_lab.changes_decision_gate,false);assert.equal(experimentRotatorContract.decision_design_lab.preregisters_plan,false);assert.equal(experimentRotatorContract.decision_design_lab.selects_version,false);
  ...
  assert.equal(experimentRotatorContract.receipt_diagnostics.unit_receipt_basis,"first_retained_receipt_per_unit");
  assert.equal(experimentRotatorContract.receipt_diagnostics.delivery_balance_basis,"matured_browser_units_only");
  assert.equal(experimentRotatorContract.receipt_diagnostics.matured_unit_delivery_interval,"wilson_score_95_percent_marginal");
  assert.equal(experimentRotatorContract.receipt_diagnostics.delivery_rate_comparison.baseline,"control");
  assert.equal(experimentRotatorContract.receipt_diagnostics.delivery_rate_comparison.effect_interval,"newcombe_wilson");
  assert.equal(experimentRotatorContract.receipt_diagnostics.delivery_rate_comparison.multiple_comparison_control,"bonferroni_two_comparisons_familywise_95_percent");
  assert.equal(experimentRotatorContract.receipt_diagnostics.intervals_support_version_selection,false);
  assert.equal(experimentRotatorContract.receipt_diagnostics.thresholds_are_operational_alerts_only,true);
  ...
  assert.deepEqual(experimentRotatorContract.results_snapshot.verifier,{version:"0.31-SVS1",module:"/experiments/wanted-result-snapshot.mjs",contract:"/experiments/snapshot-verifier.json",source_digest_field:"source_sha256",runtime_dependencies:0,performs_network_requests:false});
...
test("attributes goals across repeated sessions through one experiment unit and exposure token",()=>{
  assert.match(experimentResultsQuery,/WHERE created_at >= datetime\(\?\) AND created_at <= datetime\(\?\)/);
  assert.match(experimentResultsQuery,/WHERE g\.created_at >= datetime\(\?\) AND g\.created_at <= datetime\(\?\)/);
  assert.equal((experimentIntegrityQuery.match(/created_at >= datetime\(\?\) AND created_at <= datetime\(\?\)/g)||[]).length,1);
  assert.equal((experimentReceiptIntegrityQuery.match(/created_at>=datetime\(\?\) AND created_at<=datetime\(\?\)/g)||[]).length,1);
  assert.doesNotMatch(`${experimentResultsQuery}${experimentIntegrityQuery}${experimentReceiptIntegrityQuery}`,/datetime\('now'/);
...
test("executes the matched-exposure query against SQLite",()=>{
  ...
  const compatibleReleaseUnit=unitForVariant("control",2);event({session:"compatible_release",eventType:"experiment_exposure",variant:"control",unit:compatibleReleaseUnit,version:"0.28-R28"});event({session:"compatible_release",eventType:"experiment_goal",variant:"control",unit:compatibleReleaseUnit,version:"0.28-R28",goal:"primary_cta"});
  ...
  const futureUnit=unitForVariant("control",6);event({session:"future_session",eventType:"experiment_exposure",variant:"control",unit:futureUnit});event({session:"future_session",eventType:"experiment_goal",variant:"control",unit:futureUnit,goal:"primary_cta"});
  const oldUnit=unitForVariant("control",7);event({session:"old_session",eventType:"experiment_exposure",variant:"control",unit:oldUnit});event({session:"old_session",eventType:"experiment_goal",variant:"control",unit:oldUnit,goal:"primary_cta"});
  assert.equal(validExposureToken(controlToken)&&validExposureToken(proofToken)&&validExposureToken(developerToken),true);
  const analysisWindow=experimentAnalysisWindow(new Date(Date.now()+1_000).toISOString());
  db.prepare("UPDATE analytics_events SET created_at=datetime(?) WHERE session_id=?").run(new Date(Date.parse(analysisWindow.ended_at)+1_000).toISOString(),"future_session");
  db.prepare("UPDATE analytics_events SET created_at=datetime(?) WHERE session_id=?").run(new Date(Date.parse(analysisWindow.started_at)-1_000).toISOString(),"old_session");
  const resultParameters=[analysisWindow.started_at,analysisWindow.ended_at,"wanted_landing_v1",EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT,analysisWindow.started_at,analysisWindow.ended_at,"wanted_landing_v1",EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT];
  const rows=db.prepare(experimentResultsQuery).all(...resultParameters);
  ...
  assert.deepEqual({...db.prepare(experimentIntegrityQuery).get(analysisWindow.started_at,analysisWindow.ended_at,"wanted_landing_v1",EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT)},{cross_variant_units:1,multi_token_units:1});
  db.close();
});
...
test("reports the indexed receipt-to-exposure funnel without inventing rejected traffic",async()=>{
  const db=new DatabaseSync(":memory:");
  for(const file of ["../drizzle/0000_remarkable_mongoose.sql","../drizzle/0001_lyrical_vermin.sql"]){const migration=readFileSync(new URL(file,import.meta.url),"utf8").split("--> statement-breakpoint").join("");db.exec(migration)}
  const d1=d1FromSqlite(db),now=new Date(Date.now()+1_000),sessionA="aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",sessionB="bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",sessionC="cccccccc-cccc-4ccc-8ccc-cccccccccccc";
  ...
  const analysisWindow=experimentAnalysisWindow(now.toISOString()),cutoff=new Date(Date.parse(analysisWindow.started_at)+86_400_000).toISOString(),parameters=[cutoff,analysisWindow.ended_at,WANTED_LANDING_EXPERIMENT.id,EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT,analysisWindow.started_at,analysisWindow.ended_at,WANTED_LANDING_EXPERIMENT.id,EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT,analysisWindow.ended_at,analysisWindow.ended_at,analysisWindow.ended_at];
  const raw=db.prepare(experimentReceiptIntegrityQuery).all(...parameters),summary=summarizeReceiptIntegrity(raw);
  const byVariant=new Map(raw.map(row=>[row.variant,{...row}]));
  assert.deepEqual(byVariant.get("control"),{variant:"control",issued_receipts:1,issued_units:1,exposed_receipts:1,exposed_units:1,matured_units:0,matured_exposed_units:0,unexposed_receipts:0,expired_unexposed_receipts:0,duplicate_session_unit_receipts:0});
  assert.deepEqual(byVariant.get("proof"),{variant:"proof",issued_receipts:2,issued_units:1,exposed_receipts:0,exposed_units:0,matured_units:0,matured_exposed_units:0,unexposed_receipts:2,expired_unexposed_receipts:0,duplicate_session_unit_receipts:1});
  assert.deepEqual(byVariant.get("developer"),{variant:"developer",issued_receipts:1,issued_units:1,exposed_receipts:0,exposed_units:0,matured_units:1,matured_exposed_units:0,unexposed_receipts:1,expired_unexposed_receipts:1,duplicate_session_unit_receipts:0});
  assert.equal(summary.profile,"0.28-RD7");assert.equal(summary.receipt_followup_hours,24);assert.equal(summary.issued_receipts,4);assert.equal(summary.issued_units,3);assert.equal(summary.exposed_receipts,1);assert.equal(summary.exposed_units,1);assert.equal(summary.matured_units,1);assert.equal(summary.matured_exposed_units,0);assert.equal(summary.pending_maturity_units,2);assert.equal(summary.matured_unit_delivery_rate,0);assert.equal(summary.matured_unit_delivery_interval_95.low,0);assert.equal(summary.matured_unit_delivery_interval_95.high>0&&summary.matured_unit_delivery_interval_95.high<1,true);assert.equal(summary.unexposed_receipts,3);assert.equal(summary.active_unexposed_receipts,2);assert.equal(summary.expired_unexposed_receipts,1);assert.equal(summary.duplicate_session_unit_receipts,1);assert.equal(summary.receipt_to_exposure_rate,.25);assert.equal(summary.unit_to_exposure_rate,1/3);assert.equal(summary.variants.length,3);assert.equal(summary.delivery_rate_comparisons.length,2);assert.equal(summary.delivery_rate_comparisons.every(row=>row.interval_position==="unavailable"),true);assert.equal(summary.issuance_sample_ratio_mismatch.status,"insufficient");assert.equal(summary.unit_delivery_balance.status,"insufficient");assert.equal(summary.counts_rejected_requests,false);assert.equal(summary.proves_human_traffic,false);
  ...
  const resultParameters=[analysisWindow.started_at,analysisWindow.ended_at,WANTED_LANDING_EXPERIMENT.id,EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT,analysisWindow.started_at,analysisWindow.ended_at,WANTED_LANDING_EXPERIMENT.id,EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT];
  const resultPlan=db.prepare(`EXPLAIN QUERY PLAN ${experimentResultsQuery}`).all(...resultParameters).map(row=>String(row.detail)).join(" "),integrityPlan=db.prepare(`EXPLAIN QUERY PLAN ${experimentIntegrityQuery}`).all(analysisWindow.started_at,analysisWindow.ended_at,WANTED_LANDING_EXPERIMENT.id,EXPERIMENT_ANALYSIS_COHORT,EXPERIMENT_TREATMENT_FINGERPRINT,EXPERIMENT_PRESENTATION_FINGERPRINT).map(row=>String(row.detail)).join(" ");
  assert.match(resultPlan,/idx_analytics_event_path/);assert.match(integrityPlan,/idx_analytics_event_path/);
  const balanced=summarizeReceiptIntegrity(WANTED_LANDING_EXPERIMENT.variants.map(variant=>({variant:variant.id,issued_receipts:100,issued_units:100,exposed_receipts:80,exposed_units:80,matured_units:100,matured_exposed_units:80,unexposed_receipts:20,expired_unexposed_receipts:10,duplicate_session_unit_receipts:0})));
  assert.equal(balanced.issuance_sample_ratio_mismatch.status,"pass");assert.equal(balanced.unit_delivery_balance.status,"pass");assert.equal(balanced.matured_unit_delivery_interval_95.low<.8&&balanced.matured_unit_delivery_interval_95.high>.8,true);assert.equal(balanced.variants.every(row=>row.matured_unit_delivery_interval_95.low<.8&&row.matured_unit_delivery_interval_95.high>.8),true);assert.equal(balanced.delivery_rate_comparisons.every(row=>row.absolute_delivery_rate_difference===0&&row.familywise_interval_95.low<0&&row.familywise_interval_95.high>0&&row.interval_position==="includes_zero"),true);
  const activeSkew=summarizeReceiptIntegrity(WANTED_LANDING_EXPERIMENT.variants.map((variant,index)=>({variant:variant.id,issued_receipts:100,issued_units:100,exposed_receipts:index?64:99,exposed_units:index?64:99,matured_units:80,matured_exposed_units:64,unexposed_receipts:index?36:1,expired_unexposed_receipts:0,duplicate_session_unit_receipts:0})));
  assert.equal(activeSkew.unit_delivery_balance.status,"pass");assert.equal(activeSkew.matured_unit_delivery_rate,.8);
  const drifted=summarizeReceiptIntegrity(WANTED_LANDING_EXPERIMENT.variants.map((variant,index)=>({variant:variant.id,issued_receipts:100,issued_units:100,exposed_receipts:index?50:99,exposed_units:index?50:99,matured_units:100,matured_exposed_units:index?50:99,unexposed_receipts:index?50:1,expired_unexposed_receipts:0,duplicate_session_unit_receipts:0})));
  assert.equal(drifted.unit_delivery_balance.status,"alert");assert.equal(drifted.unit_delivery_balance.p_value<.001,true);assert.equal(drifted.delivery_rate_comparisons.every(row=>row.absolute_delivery_rate_difference===-.49&&row.familywise_interval_95.high<0&&row.interval_position==="entirely_below_zero"),true);
  const fullDelivery=summarizeReceiptIntegrity([{variant:"control",issued_receipts:10,issued_units:10,exposed_receipts:10,exposed_units:10,matured_units:10,matured_exposed_units:10}]);
  assert.equal(fullDelivery.matured_unit_delivery_interval_95.low<1,true);assert.equal(fullDelivery.matured_unit_delivery_interval_95.high>.999999999,true);
  assert.equal(summarizeReceiptIntegrity(null).matured_unit_delivery_interval_95,null);assert.equal(summarizeReceiptIntegrity([{variant:"control",issued_receipts:1,issued_units:1,exposed_receipts:4,exposed_units:4,matured_units:4,matured_exposed_units:4}]).exposed_receipts,1);db.close();
});
...
test("accepts only active-cohort, internal, schema-valid experiment events",()=>{
  ...
  const exposure={experiment:"wanted_landing_v1",analysis_cohort:EXPERIMENT_ANALYSIS_COHORT,treatment_fingerprint:EXPERIMENT_TREATMENT_FINGERPRINT,presentation_fingerprint:EXPERIMENT_PRESENTATION_FINGERPRINT,assignment_receipt:"66666666-6666-4666-8666-666666666666",unit_id:unit,variant:"control",assignment_mode:"assigned",rotator_version:"0.36-R36",exposure_id:token};
  ...
  assert.equal(validActiveCohortEventVersion("0.36-R36"),true);assert.equal(validActiveCohortEventVersion("0.35-R35"),true);assert.equal(validActiveCohortEventVersion("0.34-R34"),true);assert.equal(validActiveCohortEventVersion("0.33-R33"),true);assert.equal(validActiveCohortEventVersion("0.32-R32"),true);assert.equal(validActiveCohortEventVersion("0.31-R31"),true);assert.equal(validActiveCohortEventVersion("0.30-R30"),true);assert.equal(validActiveCohortEventVersion("0.29-R29"),true);assert.equal(validActiveCohortEventVersion("0.28-R28"),true);assert.equal(validActiveCohortEventVersion("0.27-R27"),false);assert.equal(validActiveCohortEventVersion("0.37-R37"),false);
  ...
  assert.equal(validExperimentEvent("experiment_exposure","/wanted-10k",{...exposure,rotator_version:"0.32-R32"}),true);
  assert.equal(validExperimentEvent("experiment_exposure","/wanted-10k",{...exposure,rotator_version:"0.37-R37"}),false);
