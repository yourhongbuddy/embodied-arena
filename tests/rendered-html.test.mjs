...
test("publishes signed telemetry root envelopes behind witnessed digests",async()=>{
  const responses=await Promise.all([request("/wanted-10k/root-commitment-witness/envelopes"),request("/wanted-10k/root-envelope.json","application/json"),request("/wanted-10k/root-envelope.schema.json","application/json"),request("/wanted-10k/root-envelope-batch.schema.json","application/json"),request("/wanted-10k/root-envelope.template.json","application/json"),request("/wanted-10k/wanted-root-envelope.mjs"),request("/wanted-10k/root-envelope-sdk.json","application/json"),request("/wanted-10k/spec.json","application/json"),request("/wanted-10k/openapi.json","application/json"),request("/wanted-10k/protocol")]);
  for(const response of responses)assert.equal(response.status,200);
  const [page,profile,schema,batchSchema,template,moduleSource,sdkContract,spec,openapi,protocol]=await Promise.all([responses[0].text(),responses[1].json(),responses[2].json(),responses[3].json(),responses[4].json(),responses[5].text(),responses[6].json(),responses[7].json(),responses[8].json(),responses[9].text()]);
  assert.match(page,/A digest of what/);assert.match(page,/Tail binds prefix/);assert.match(page,/ROOT INTEGRITY != CAPTURE COMPLETENESS/);assert.match(page,/--verify-series/);assert.match(page,/--verify-batch/);assert.match(page,/wanted-root-envelope\.mjs/);
  assert.equal(profile.version,"0.2-RE1");assert.equal(profile.batch_verification_profile,"0.2-REB1");assert.equal(profile.telemetry_authenticity_profile,"0.2-T1");assert.equal(profile.certification_effect,"hard_field_gate");assert.equal(profile.ranking_effect,"eligibility_only_not_score_or_tiebreaker");assert.equal(profile.audit_summary.field,"root_envelope_integrity");assert.equal(profile.audit_summary.verification_profile,"0.2-REB1");assert.equal(profile.cadence.zero_event_intervals_permitted,true);assert.equal(schema.additionalProperties,false);assert.equal(schema.required.includes("previous_root_commitment_sha256"),true);assert.equal(batchSchema.properties.profile_version.const,"0.2-REB1");
  ...
  assert.match(moduleSource,/verifyRootEnvelopeSeries/);assert.match(moduleSource,/verifyRootEnvelopeBatch/);assert.match(responses[5].headers.get("content-disposition"),/wanted-root-envelope\.mjs/);assert.equal(sdkContract.version,"0.3-RES2");assert.equal(sdkContract.batch_profile,"0.2-REB1");assert.deepEqual(sdkContract.runtime_dependencies,["./wanted-telemetry-verifier.mjs"]);assert.equal(sdkContract.performs_network_requests,false);assert.match(sdkContract.source_sha256,/^[a-f0-9]{64}$/);
  assert.equal(spec.root_envelope_profile.version,"0.2-RE1");assert.equal(spec.root_envelope_profile.batch_verification_profile,"0.2-REB1");assert.equal(spec.cohort.rankability_requires.includes("root_envelope_0.2-RE1_passed"),true);assert.equal(spec.cohort.rankability_requires.includes("root_envelope_batch_0.2-REB1_passed"),true);assert.equal(spec.certification_handoff.binds.includes("root_envelope_profile_0.2-RE1"),true);assert.equal(spec.certification_handoff.binds.includes("root_envelope_batch_profile_0.2-REB1"),true);assert.equal(spec.root_envelope_sdk_profile.version,"0.3-RES2");assert.equal(spec.developer_resources.root_envelope_guide,"/wanted-10k/root-commitment-witness/envelopes");assert.equal(spec.developer_resources.root_envelope_batch_schema,"/wanted-10k/root-envelope-batch.schema.json");assert.equal(openapi["x-wanted-root-envelope"].version,"0.2-RE1");assert.equal(openapi["x-wanted-root-envelope"].batch_verification_profile,"0.2-REB1");assert.equal(openapi["x-wanted-root-envelope"].certification_effect,"hard_field_gate");assert.equal(openapi["x-wanted-root-envelope"].audit_manifest_field,"root_envelope_integrity");assert.equal(openapi["x-wanted-root-envelope"].performs_network_requests,false);assert.match(protocol,/SIGNED ROOT ENVELOPES/);assert.match(protocol,/require that proof for every field certification/i);assert.match(protocol,/root-envelope-batch\.schema\.json/);assert.match(protocol,/root-envelope-sdk\.json/);
});
