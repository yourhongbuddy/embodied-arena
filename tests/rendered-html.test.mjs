...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/ANALYSIS WINDOW/);assert.match(pageHtml,/exact inclusive 30-day server-UTC window/i);assert.match(pageHtml,/used by every experiment and receipt query/i);
  ...
  assert.equal(contract.analysis_cohort.previous_cohorts.at(-1).id,"wanted_landing_v1-C6");assert.equal(contract.analysis_cohort.previous_cohorts.at(-1).ends_at_implementation_version,"0.26-R26");assert.equal(contract.analysis_cohort.previous_cohorts.at(-1).reason_closed,"explicit_server_utc_analysis_window_boundaries_added");
  assert.equal(contract.counting.reporting_window_days,30);assert.equal(contract.counting.reporting_window.clock,"server_utc");assert.equal(contract.counting.reporting_window.timestamp_source,"one_route_handler_snapshot");assert.equal(contract.counting.reporting_window.timestamp_precision,"whole_seconds_matching_d1_current_timestamp");assert.equal(contract.counting.reporting_window.start_inclusive,true);assert.equal(contract.counting.reporting_window.end_inclusive,true);assert.equal(contract.counting.reporting_window.all_event_queries_share_boundaries,true);assert.equal(contract.counting.reporting_window.sqlite_now_functions_used,false);
  assert.equal(contract.receipt_diagnostics.analysis_window,"shared_top_level_server_utc_window");
  assert.equal(contract.receipt_diagnostics.profile,"0.27-RD6");assert.equal(contract.receipt_diagnostics.grouping,"assigned_version");assert.equal(contract.receipt_diagnostics.unit_receipt_basis,"first_retained_receipt_per_unit");assert.equal(contract.receipt_diagnostics.delivery_balance_basis,"matured_browser_units_only");assert.equal(contract.receipt_diagnostics.matured_unit_delivery_interval,"wilson_score_95_percent_marginal");assert.equal(contract.receipt_diagnostics.delivery_rate_comparison.baseline,"control");assert.equal(contract.receipt_diagnostics.delivery_rate_comparison.effect_measure,"absolute_matured_unit_delivery_rate_difference");assert.equal(contract.receipt_diagnostics.delivery_rate_comparison.effect_interval,"newcombe_wilson");assert.equal(contract.receipt_diagnostics.delivery_rate_comparison.multiple_comparison_control,"bonferroni_two_comparisons_familywise_95_percent");assert.equal(contract.receipt_diagnostics.intervals_support_version_selection,false);assert.equal(contract.receipt_diagnostics.thresholds_are_operational_alerts_only,true);assert.equal(contract.receipt_diagnostics.decision_effect,"none");assert.equal(contract.receipt_diagnostics.counts_rejected_requests,false);
  assert.equal(results.experiment,"wanted_landing_v1");assert.equal(results.analysis_cohort,"wanted_landing_v1-C7");assert.equal(results.treatment_fingerprint,contract.analysis_cohort.treatment_fingerprint);assert.equal(results.presentation_fingerprint,contract.analysis_cohort.presentation_fingerprint);assert.equal(results.assignment_receipt_profile,"0.21-AR1");assert.equal(results.assignment_receipt_required,true);assert.equal(results.receipt_integrity.profile,"0.27-RD6");assert.equal(results.receipt_integrity.status,"insufficient");assert.equal(results.receipt_integrity.receipt_to_exposure_rate,null);assert.equal(results.receipt_integrity.unit_to_exposure_rate,null);assert.equal(results.receipt_integrity.receipt_followup_hours,24);assert.equal(results.receipt_integrity.matured_units,0);assert.equal(results.receipt_integrity.matured_exposed_units,0);assert.equal(results.receipt_integrity.pending_maturity_units,0);assert.equal(results.receipt_integrity.matured_unit_delivery_rate,null);assert.equal(results.receipt_integrity.matured_unit_delivery_interval_95,null);assert.equal(results.receipt_integrity.variants.length,3);assert.equal(results.receipt_integrity.delivery_rate_comparisons.length,2);assert.equal(results.receipt_integrity.delivery_rate_comparisons.every(comparison=>comparison.interval_position==="unavailable"),true);assert.equal(results.receipt_integrity.issuance_sample_ratio_mismatch.status,"insufficient");assert.equal(results.receipt_integrity.unit_delivery_balance.status,"insufficient");assert.equal(results.receipt_integrity.counts_rejected_requests,false);assert.equal(results.receipt_integrity.proves_human_traffic,false);assert.equal(results.implementation_version,"0.27-R27");assert.equal(results.analysis_unit,"experiment_scoped_anonymous_browser_unit");assert.equal(results.unit_represents,"one_first_party_browser_profile_storage_instance");assert.equal(results.reported_as_unique_users,false);assert.equal(results.human_identity_resolution,false);assert.equal(results.variants.length,3);assert.equal(results.comparisons.length,2);assert.equal(results.comparisons.every(comparison=>comparison.interval_position==="unavailable"),true);assert.equal(results.decision_gate.status,"descriptive_only");assert.equal(results.decision_gate.automatic_action,false);assert.deepEqual(results.decision_gate.blocking_reasons,["rolling_window_continuously_monitored","no_repeated_look_adjustment","no_preregistered_stopping_rule","human_traffic_not_authenticated"]);assert.equal(results.sample_ratio_mismatch.status,"insufficient");assert.equal(Number.isInteger(results.cross_variant_units_excluded),true);assert.equal(Number.isInteger(results.multi_token_units_excluded),true);assert.equal(Number.isInteger(results.total_exposed_units),true);assert.equal(["ready","unavailable"].includes(results.status),true);
  assert.equal(results.analysis_window.profile,"0.27-AW1");assert.equal(results.analysis_window.clock,"server_utc");assert.equal(results.analysis_window.precision,"whole_seconds");assert.equal(results.analysis_window.start_inclusive,true);assert.equal(results.analysis_window.end_inclusive,true);assert.equal(results.analysis_window.duration_days,30);assert.equal(Date.parse(results.analysis_window.ended_at)-Date.parse(results.analysis_window.started_at),30*86_400_000);assert.equal(new Date(results.analysis_window.started_at).toISOString(),results.analysis_window.started_at);assert.equal(new Date(results.analysis_window.ended_at).toISOString(),results.analysis_window.ended_at);
  const baseUrl=await standaloneServer();
  ...
  const metadata={experiment:"wanted_landing_v1",analysis_cohort:"wanted_landing_v1-C7",treatment_fingerprint:contract.analysis_cohort.treatment_fingerprint,presentation_fingerprint:contract.analysis_cohort.presentation_fingerprint,assignment_receipt:"88888888-8888-4888-8888-888888888888",unit_id:unit,variant:assignment.variant,assignment_mode:"assigned",rotator_version:"0.27-R27",exposure_id:token};
