...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.equal(contract.decision_design_lab.profile,"0.34-DL1");assert.equal(contract.decision_design_lab.route,"/experiments/design-lab");assert.equal(contract.decision_design_lab.contract,"/experiments/design-lab.json");assert.equal(contract.decision_design_lab.changes_live_allocation,false);assert.equal(contract.decision_design_lab.changes_live_assignment,false);assert.equal(contract.decision_design_lab.changes_decision_gate,false);assert.equal(contract.decision_design_lab.preregisters_plan,false);assert.equal(contract.decision_design_lab.selects_version,false);
  ...
  assert.match(rotatorModuleResponse.headers.get("content-type"),/text\/javascript/);assert.equal(rotatorContract.version,"0.32-RSDK1");assert.equal(rotatorContract.rotator_version,"0.34-R34");assert.deepEqual(rotatorContract.compatible_rotator_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34"]);assert.equal(rotatorContract.lab,"/experiments/assignment-lab");assert.equal(rotatorContract.design_lab,"/experiments/design-lab");assert.equal(rotatorContract.runtime_dependencies,0);assert.equal(rotatorContract.performs_network_requests,false);assert.equal(rotatorContract.source_sha256,createHash("sha256").update(rotatorSource).digest("hex"));
