...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.equal(contract.version,"0.4-R4");assert.equal(contract.assignment.stable_per_device,true);assert.equal(contract.assignment.pre_assignment_presentation,"neutral_noninteractive");assert.equal(contract.privacy.event_retention_days,35);assert.equal(contract.safety_boundary.changes_score,false);assert.equal(contract.inference.winner_declaration,false);assert.equal(contract.experiments[0].variants.reduce((sum,variant)=>sum+variant.weight_basis_points,0),10_000);
  ...
  const accepted=await fetch(new URL("api/analytics",baseUrl),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:"session_test_001",eventType:"experiment_exposure",path:"/wanted-10k",metadata:{experiment:"wanted_landing_v1",variant:"control",assignment_mode:"assigned",rotator_version:"0.4-R4"}})});
  assert.equal(accepted.status,204);
  const forged=await fetch(new URL("api/analytics",baseUrl),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:"session_test_001",eventType:"experiment_exposure",path:"/wanted-10k",metadata:{experiment:"wanted_landing_v1",variant:"winner",assignment_mode:"assigned",rotator_version:"0.4-R4"}})});
