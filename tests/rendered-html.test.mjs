...
test("publishes independent periodic root-commitment witnessing",async()=>{
  ...
  const [contract,schema,template,audit,leaderboard,spec,openapi,moduleSource,sdkContract]=await Promise.all([responses[1].json(),responses[2].json(),responses[3].json(),responses[4].json(),responses[5].json(),responses[6].json(),responses[7].json(),responses[8].text(),responses[9].json()]);
  ...
  assert.equal(spec.root_commitment_witness_profile.quorum.minimum_independent_organizations_per_root,2);assert.equal(spec.root_commitment_witness_profile.local_verifier.pasted_manifest_uploads,false);assert.equal(spec.root_witness_verifier_sdk_profile.version,"0.2-RCS1");assert.equal(spec.root_witness_verifier_sdk_profile.cli.exit_codes.verification_failed,1);assert.equal(spec.developer_resources.root_commitment_witness_lab,"/wanted-10k/root-commitment-witness");assert.equal(spec.developer_resources.root_witness_verifier_module,"/wanted-10k/wanted-root-witness-verifier.mjs");
  ...
  assert.equal(openapi["x-wanted-root-witness-verifier"].version,"0.2-RCS1");assert.equal(openapi["x-wanted-root-witness-verifier"].performs_network_requests,false);assert.equal(sdkContract.version,"0.2-RCS1");assert.equal(sdkContract.performs_network_requests,false);assert.match(sdkContract.source_sha256,/^[a-f0-9]{64}$/);assert.match(moduleSource,/verifyRootWitnessManifest/);assert.match(responses[8].headers.get("content-disposition"),/wanted-root-witness-verifier\.mjs/);
});
