...
test("publishes signed telemetry root envelopes behind witnessed digests",async()=>{
  ...
  assert.equal(profile.version,"0.2-RE1");assert.equal(profile.telemetry_authenticity_profile,"0.2-T1");assert.equal(profile.cadence.zero_event_intervals_permitted,true);assert.equal(schema.additionalProperties,false);assert.equal(schema.required.includes("previous_root_commitment_sha256"),true);
  ...
  assert.equal(spec.root_envelope_profile.version,"0.2-RE1");assert.equal(spec.root_envelope_sdk_profile.version,"0.2-RES1");assert.equal(spec.developer_resources.root_envelope_guide,"/wanted-10k/root-commitment-witness/envelopes");assert.equal(openapi["x-wanted-root-envelope"].version,"0.2-RE1");assert.equal(openapi["x-wanted-root-envelope"].performs_network_requests,false);assert.match(protocol,/SIGNED ROOT ENVELOPES/);assert.match(protocol,/root-envelope-sdk\.json/);
});
