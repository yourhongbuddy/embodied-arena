...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/ANALYSIS WINDOW/);assert.match(pageHtml,/RESULT SNAPSHOT/);assert.match(pageHtml,/ROTATOR SDK/);assert.match(pageHtml,/ASSIGNMENT LAB/);assert.match(pageHtml,/SNAPSHOT VERIFIER/);assert.match(pageHtml,/canonical SHA-256 content digest/i);assert.match(pageHtml,/not a signature/i);assert.match(pageHtml,/Unavailable responses publish no synthetic snapshot/i);assert.match(pageHtml,/exact inclusive 30-day D1 database-UTC window/i);assert.match(pageHtml,/unavailable results do not invent a window/i);
  ...
  assert.match(rotatorModuleResponse.headers.get("content-type"),/text\/javascript/);assert.equal(rotatorContract.version,"0.32-RSDK1");assert.equal(rotatorContract.rotator_version,"0.33-R33");assert.deepEqual(rotatorContract.compatible_rotator_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33"]);assert.equal(rotatorContract.lab,"/experiments/assignment-lab");assert.equal(rotatorContract.runtime_dependencies,0);assert.equal(rotatorContract.performs_network_requests,false);assert.equal(rotatorContract.source_sha256,createHash("sha256").update(rotatorSource).digest("hex"));
