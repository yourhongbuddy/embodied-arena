...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.equal(contract.version,"0.10-R10");assert.equal(contract.assignment.stable_per_device,true);assert.equal(contract.assignment.one_assigned_variant_per_session,true);assert.equal(contract.assignment.pre_assignment_presentation,"neutral_noninteractive");assert.equal(contract.privacy.event_retention_days,35);assert.equal(contract.delivery.session_marker_after_acknowledgement,true);assert.equal(contract.delivery.goal_outbox_scope,"session_only");assert.equal(contract.counting.receipt_order_dependency,false);assert.equal(contract.counting.cross_variant_sessions_excluded,true);assert.equal(contract.counting.multi_token_sessions_excluded,true);assert.equal(contract.counting.one_exposure_token_per_counted_session,true);assert.equal(contract.safety_boundary.changes_score,false);assert.equal(contract.inference.multiple_comparison_control,"bonferroni_two_comparisons_familywise_95_percent");assert.equal(contract.inference.winner_declaration,false);assert.equal(contract.experiments[0].variants.reduce((sum,variant)=>sum+variant.weight_basis_points,0),10_000);
  assert.equal(results.experiment,"wanted_landing_v1");assert.equal(results.variants.length,3);assert.equal(results.comparisons.length,2);assert.equal(results.comparisons.every(comparison=>comparison.signal==="insufficient"),true);assert.equal(results.sample_ratio_mismatch.status,"insufficient");assert.equal(Number.isInteger(results.cross_variant_sessions_excluded),true);assert.equal(Number.isInteger(results.multi_token_sessions_excluded),true);assert.equal(["ready","unavailable"].includes(results.status),true);
  const baseUrl=await standaloneServer();
