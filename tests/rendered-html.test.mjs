...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/Test the framing/);assert.match(pageHtml,/PRESENTATION ONLY/);assert.match(pageHtml,/experiment-only unit ID/);assert.match(pageHtml,/rolling 35-day raw-event window/);assert.match(pageHtml,/wanted_variant=control/);assert.match(pageHtml,/wanted_variant=proof/);assert.match(pageHtml,/wanted_variant=developer/);
  const [contract,results]=await Promise.all([contractResponse.json(),resultResponse.json()]);
  assert.equal(contract.version,"0.11-R11");assert.equal(contract.assignment.stable_per_device,true);assert.equal(contract.assignment.analysis_unit_matches_assignment_unit,true);assert.equal(contract.assignment.one_assigned_variant_per_session,true);assert.equal(contract.assignment.pre_assignment_presentation,"neutral_noninteractive");assert.equal(contract.privacy.event_retention_days,35);assert.equal(contract.privacy.raw_assignment_seed_transmitted,false);assert.equal(contract.privacy.unit_id_cross_experiment_linkage,false);assert.equal(contract.delivery.session_marker_after_acknowledgement,true);assert.equal(contract.delivery.goal_outbox_scope,"session_only");assert.equal(contract.counting.receipt_order_dependency,false);assert.equal(contract.counting.cross_variant_units_excluded,true);assert.equal(contract.counting.multi_token_units_excluded,true);assert.equal(contract.counting.one_exposure_token_per_counted_unit,true);assert.equal(contract.safety_boundary.changes_score,false);assert.equal(contract.inference.multiple_comparison_control,"bonferroni_two_comparisons_familywise_95_percent");assert.equal(contract.inference.winner_declaration,false);assert.equal(contract.experiments[0].variants.reduce((sum,variant)=>sum+variant.weight_basis_points,0),10_000);
  assert.equal(results.experiment,"wanted_landing_v1");assert.equal(results.variants.length,3);assert.equal(results.comparisons.length,2);assert.equal(results.comparisons.every(comparison=>comparison.signal==="insufficient"),true);assert.equal(results.sample_ratio_mismatch.status,"insufficient");assert.equal(Number.isInteger(results.cross_variant_units_excluded),true);assert.equal(Number.isInteger(results.multi_token_units_excluded),true);assert.equal(Number.isInteger(results.total_exposed_units),true);assert.equal(["ready","unavailable"].includes(results.status),true);
  const baseUrl=await standaloneServer();
  ...
  const accepted=await fetch(new URL("api/analytics",baseUrl),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:"session_test_001",eventType:"experiment_exposure",path:"/wanted-10k",metadata:{experiment:"wanted_landing_v1",unit_id:"10101010-1010-4010-8010-101010101010",variant:"control",assignment_mode:"assigned",rotator_version:"0.11-R11",exposure_id:"11111111-1111-4111-8111-111111111111"}})});
  assert.equal(accepted.status,204);assert.equal(["accepted","unavailable"].includes(accepted.headers.get("x-analytics-status")),true);
  const forged=await fetch(new URL("api/analytics",baseUrl),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:"session_test_001",eventType:"experiment_exposure",path:"/wanted-10k",metadata:{experiment:"wanted_landing_v1",unit_id:"10101010-1010-4010-8010-101010101010",variant:"winner",assignment_mode:"assigned",rotator_version:"0.11-R11",exposure_id:"11111111-1111-4111-8111-111111111111"}})});
