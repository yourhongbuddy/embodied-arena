...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/ANALYSIS WINDOW/);assert.match(pageHtml,/RESULT SNAPSHOT/);assert.match(pageHtml,/ROTATOR SDK/);assert.match(pageHtml,/SNAPSHOT VERIFIER/);assert.match(pageHtml,/canonical SHA-256 content digest/i);assert.match(pageHtml,/not a signature/i);assert.match(pageHtml,/Unavailable responses publish no synthetic snapshot/i);assert.match(pageHtml,/exact inclusive 30-day D1 database-UTC window/i);assert.match(pageHtml,/unavailable results do not invent a window/i);
  ...
  assert.equal(contract.assignment_sdk.version,"0.32-RSDK1");assert.equal(contract.assignment_sdk.module,"/experiments/wanted-rotator.mjs");assert.equal(contract.assignment_sdk.contract,"/experiments/rotator-sdk.json");assert.equal(contract.assignment_sdk.conformance_vectors.length,3);assert.equal(contract.assignment_sdk.runtime_dependencies,0);assert.equal(contract.assignment_sdk.performs_network_requests,false);assert.equal(contract.assignment_sdk.changes_assignment,false);
  ...
  assert.match(rotatorModuleResponse.headers.get("content-type"),/text\/javascript/);assert.equal(rotatorContract.version,"0.32-RSDK1");assert.equal(rotatorContract.rotator_version,"0.32-R32");assert.equal(rotatorContract.runtime_dependencies,0);assert.equal(rotatorContract.performs_network_requests,false);assert.equal(rotatorContract.source_sha256,createHash("sha256").update(rotatorSource).digest("hex"));
