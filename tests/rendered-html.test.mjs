...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.equal(contract.receipt_diagnostics.profile,"0.24-RD3");assert.equal(contract.receipt_diagnostics.grouping,"assigned_version");assert.equal(contract.receipt_diagnostics.unit_receipt_basis,"first_retained_receipt_per_unit");assert.equal(contract.receipt_diagnostics.delivery_balance_basis,"matured_browser_units_only");assert.equal(contract.receipt_diagnostics.thresholds_are_operational_alerts_only,true);assert.equal(contract.receipt_diagnostics.decision_effect,"none");assert.equal(contract.receipt_diagnostics.counts_rejected_requests,false);
  assert.equal(results.experiment,"wanted_landing_v1");assert.equal(results.analysis_cohort,"wanted_landing_v1-C6");assert.equal(results.treatment_fingerprint,contract.analysis_cohort.treatment_fingerprint);assert.equal(results.presentation_fingerprint,contract.analysis_cohort.presentation_fingerprint);assert.equal(results.assignment_receipt_profile,"0.21-AR1");assert.equal(results.assignment_receipt_required,true);assert.equal(results.receipt_integrity.profile,"0.24-RD3");assert.equal(results.receipt_integrity.status,"insufficient");assert.equal(results.receipt_integrity.receipt_to_exposure_rate,null);assert.equal(results.receipt_integrity.unit_to_exposure_rate,null);assert.equal(results.receipt_integrity.receipt_followup_hours,24);assert.equal(results.receipt_integrity.matured_units,0);assert.equal(results.receipt_integrity.matured_exposed_units,0);assert.equal(results.receipt_integrity.pending_maturity_units,0);assert.equal(results.receipt_integrity.matured_unit_delivery_rate,null);assert.equal(results.receipt_integrity.variants.length,3);assert.equal(results.receipt_integrity.issuance_sample_ratio_mismatch.status,"insufficient");assert.equal(results.receipt_integrity.unit_delivery_balance.status,"insufficient");assert.equal(results.receipt_integrity.counts_rejected_requests,false);assert.equal(results.receipt_integrity.proves_human_traffic,false);assert.equal(results.implementation_version,"0.24-R24");assert.equal(results.analysis_unit,"experiment_scoped_anonymous_browser_unit");assert.equal(results.unit_represents,"one_first_party_browser_profile_storage_instance");assert.equal(results.reported_as_unique_users,false);assert.equal(results.human_identity_resolution,false);assert.equal(results.variants.length,3);assert.equal(results.comparisons.length,2);assert.equal(results.comparisons.every(comparison=>comparison.interval_position==="unavailable"),true);assert.equal(results.decision_gate.status,"descriptive_only");assert.equal(results.decision_gate.automatic_action,false);assert.deepEqual(results.decision_gate.blocking_reasons,["rolling_window_continuously_monitored","no_repeated_look_adjustment","no_preregistered_stopping_rule","human_traffic_not_authenticated"]);assert.equal(results.sample_ratio_mismatch.status,"insufficient");assert.equal(Number.isInteger(results.cross_variant_units_excluded),true);assert.equal(Number.isInteger(results.multi_token_units_excluded),true);assert.equal(Number.isInteger(results.total_exposed_units),true);assert.equal(["ready","unavailable"].includes(results.status),true);
  const baseUrl=await standaloneServer();
