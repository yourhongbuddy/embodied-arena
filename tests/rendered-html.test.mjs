...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.equal(contract.version,"0.6-R6");assert.equal(contract.assignment.stable_per_device,true);assert.equal(contract.assignment.pre_assignment_presentation,"neutral_noninteractive");assert.equal(contract.privacy.event_retention_days,35);assert.equal(contract.delivery.session_marker_after_acknowledgement,true);assert.equal(contract.counting.receipt_order_dependency,false);assert.equal(contract.safety_boundary.changes_score,false);assert.equal(contract.inference.winner_declaration,false);assert.equal(contract.experiments[0].variants.reduce((sum,variant)=>sum+variant.weight_basis_points,0),10_000);
