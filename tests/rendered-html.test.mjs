...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/Test the framing/);assert.match(pageHtml,/PRESENTATION ONLY/);assert.match(pageHtml,/DESCRIPTIVE ONLY/);assert.match(pageHtml,/INTERVAL POSITION/);assert.match(pageHtml,/cannot select or stop a version/i);assert.match(pageHtml,/experiment-only assignment ID/);assert.match(pageHtml,/browser-profile units, not unique people, households, or devices/i);assert.match(pageHtml,/shared profile can combine people/i);assert.match(pageHtml,/site opt-out, Global Privacy Control, or Do Not Track signal/i);assert.match(pageHtml,/DISABLE LOCAL ANALYTICS/);assert.match(pageHtml,/server recomputes each eligible unit.*assigned version and cohort-bound exposure token/i);assert.match(pageHtml,/SHA-256 treatment fingerprint/i);assert.match(pageHtml,/rendered-source fingerprint/i);assert.match(pageHtml,/component, layout, navigation, or CSS drift cannot mix into it/i);assert.match(pageHtml,/compatible non-presentation releases do not restart the test/i);assert.match(pageHtml,/rolling 35-day raw-event window/);assert.match(pageHtml,/wanted_variant=control/);assert.match(pageHtml,/wanted_variant=proof/);assert.match(pageHtml,/wanted_variant=developer/);
  ...
  const metadata={experiment:"wanted_landing_v1",analysis_cohort:"wanted_landing_v1-C5",treatment_fingerprint:contract.analysis_cohort.treatment_fingerprint,presentation_fingerprint:contract.analysis_cohort.presentation_fingerprint,unit_id:unit,variant:assignment.variant,assignment_mode:"assigned",rotator_version:"0.20-R20",exposure_id:token};
