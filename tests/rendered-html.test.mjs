...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/ANALYSIS WINDOW/);assert.match(pageHtml,/RESULT SNAPSHOT/);assert.match(pageHtml,/ROTATOR SDK/);assert.match(pageHtml,/ASSIGNMENT LAB/);assert.match(pageHtml,/DESIGN LAB/);assert.match(pageHtml,/SNAPSHOT VERIFIER/);assert.match(pageHtml,/canonical SHA-256 content digest/i);assert.match(pageHtml,/not a signature/i);assert.match(pageHtml,/Unavailable responses publish no synthetic snapshot/i);assert.match(pageHtml,/exact inclusive 30-day D1 database-UTC window/i);assert.match(pageHtml,/unavailable results do not invent a window/i);
  ...
  assert.equal(contract.decision_design_lab.profile,"0.34-DL1");assert.equal(contract.decision_design_lab.route,"/experiments/design-lab");assert.equal(contract.decision_design_lab.contract,"/experiments/design-lab.json");assert.equal(contract.decision_design_lab.plan_schema,"/experiments/design-plan.schema.json");assert.equal(contract.decision_design_lab.reference_plan,"/experiments/design-plan.reference.json");assert.deepEqual(contract.decision_design_lab.verifier,{profile:"0.35-DPV1",module:"/experiments/wanted-design-plan.mjs",contract:"/experiments/design-plan-verifier.json",runtime_dependencies:0,performs_network_requests:false});assert.equal(contract.decision_design_lab.changes_live_allocation,false);assert.equal(contract.decision_design_lab.changes_live_assignment,false);assert.equal(contract.decision_design_lab.changes_decision_gate,false);assert.equal(contract.decision_design_lab.preregisters_plan,false);assert.equal(contract.decision_design_lab.selects_version,false);
  ...
  assert.match(rotatorModuleResponse.headers.get("content-type"),/text\/javascript/);assert.equal(rotatorContract.version,"0.32-RSDK1");assert.equal(rotatorContract.rotator_version,"0.35-R35");assert.deepEqual(rotatorContract.compatible_rotator_versions,["0.28-R28","0.29-R29","0.30-R30","0.31-R31","0.32-R32","0.33-R33","0.34-R34","0.35-R35"]);assert.equal(rotatorContract.lab,"/experiments/assignment-lab");assert.equal(rotatorContract.design_lab,"/experiments/design-lab");assert.equal(rotatorContract.runtime_dependencies,0);assert.equal(rotatorContract.performs_network_requests,false);assert.equal(rotatorContract.source_sha256,createHash("sha256").update(rotatorSource).digest("hex"));
...
test("publishes a portable design-plan verification contract",async()=>{
  ...
  assert.equal(reference.canonical_plan_sha256,"9d3226385c4cbb34ea7aee672f858457d412cf245ed4682b2fdf42c02f929cf5");assert.equal(reference.plan.rotator_version,"0.35-R35");assert.match(reference.interpretation,/not an externally preregistered experiment design/i);assert.match(reference.plan.selection_rule,/no automatic winner/i);assert.equal(reference.plan.current_site_decision_gate.automatic_action,false);
});
