...
test("publishes independent periodic root-commitment witnessing",async()=>{
  ...
  const page=await responses[0].text();assert.match(page,/Commit first/);assert.match(page,/No post-hoc roots/);assert.match(page,/ELIGIBILITY ONLY/);
  ...
  assert.equal(contract.version,"0.2-RC1");assert.equal(contract.witness_quorum.minimum_organizations_per_root,2);assert.equal(contract.publication_delay.maximum_hours,24);
  ...
  assert.equal(spec.root_commitment_witness_profile.quorum.minimum_independent_organizations_per_root,2);assert.equal(spec.developer_resources.root_commitment_witness_lab,"/wanted-10k/root-commitment-witness");
