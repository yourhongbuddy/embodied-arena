...
test("publishes a stable privacy-first site version rotator",async()=>{
  ...
  assert.match(pageHtml,/Test the framing/);assert.match(pageHtml,/PRESENTATION ONLY/);assert.match(pageHtml,/DESCRIPTIVE ONLY/);assert.match(pageHtml,/INTERVAL POSITION/);assert.match(pageHtml,/cannot select or stop a version/i);assert.match(pageHtml,/experiment-only assignment ID/);assert.match(pageHtml,/server recomputes the assigned version and cohort-bound exposure token/i);assert.match(pageHtml,/compatible software releases do not restart the test/i);assert.match(pageHtml,/rolling 35-day raw-event window/);assert.match(pageHtml,/wanted_variant=control/);assert.match(pageHtml,/wanted_variant=proof/);assert.match(pageHtml,/wanted_variant=developer/);
  const [contract,results]=await Promise.all([contractResponse.json(),resultResponse.json()]);
  assert.equal(contract.version,"0.14-R14");assert.equal(contract.analysis_cohort.id,"wanted_landing_v1-C1");assert.equal(contract.analysis_cohort.starts_at_implementation_version,"0.14-R14");assert.deepEqual(contract.analysis_cohort.legacy_versions_included,[]);assert.equal(contract.analysis_cohort.implementation_revision_changes_reset_cohort,false);assert.equal(contract.assignment.stable_per_device,true);assert.equal(contract.assignment.server_recomputable,true);assert.equal(contract.assignment.analysis_unit_matches_assignment_unit,true);assert.equal(contract.assignment.one_assigned_variant_per_session,true);assert.equal(contract.assignment.pre_assignment_presentation,"neutral_noninteractive");assert.equal(contract.privacy.event_retention_days,35);assert.equal(contract.privacy.assignment_id_contains_user_attributes,false);assert.equal(contract.privacy.separate_device_identifier,false);assert.equal(contract.privacy.unit_id_cross_experiment_linkage,false);assert.equal(contract.delivery.exposure_token_server_recomputable,true);assert.equal(contract.delivery.exposure_token_implementation_revision_bound,false);assert.equal(contract.delivery.session_marker_after_acknowledgement,true);assert.equal(contract.delivery.goal_outbox_scope,"session_only");assert.equal(contract.counting.receipt_order_dependency,false);assert.equal(contract.counting.cross_variant_units_excluded,true);assert.equal(contract.counting.multi_token_units_excluded,true);assert.equal(contract.counting.one_exposure_token_per_counted_unit,true);assert.equal(contract.ingestion.analysis_cohort_required,true);assert.equal(contract.ingestion.assigned_mode_only,true);assert.equal(contract.ingestion.server_recomputes_variant,true);assert.equal(contract.ingestion.server_recomputes_exposure_token,true);assert.equal(contract.ingestion.automated_fabrication_resistance,false);assert.equal(contract.ingestion.decision_use_without_edge_abuse_control,false);assert.equal(contract.safety_boundary.changes_score,false);assert.equal(contract.inference.multiple_comparison_control,"bonferroni_two_comparisons_familywise_95_percent");assert.equal(contract.inference.monitoring_window,"rolling_30_day_continuously_viewed");assert.equal(contract.inference.repeated_look_adjustment,"none");assert.equal(contract.inference.confidence_intervals_support_stopping,false);assert.equal(contract.inference.winner_declaration,false);assert.equal(contract.experiments[0].variants.reduce((sum,variant)=>sum+variant.weight_basis_points,0),10_000);
  assert.equal(results.experiment,"wanted_landing_v1");assert.equal(results.analysis_cohort,"wanted_landing_v1-C1");assert.equal(results.implementation_version,"0.14-R14");assert.equal(results.variants.length,3);assert.equal(results.comparisons.length,2);assert.equal(results.comparisons.every(comparison=>comparison.interval_position==="unavailable"),true);assert.equal(results.decision_gate.status,"descriptive_only");assert.equal(results.decision_gate.automatic_action,false);assert.deepEqual(results.decision_gate.blocking_reasons,["rolling_window_continuously_monitored","no_repeated_look_adjustment","no_preregistered_stopping_rule","human_traffic_not_authenticated"]);assert.equal(results.sample_ratio_mismatch.status,"insufficient");assert.equal(Number.isInteger(results.cross_variant_units_excluded),true);assert.equal(Number.isInteger(results.multi_token_units_excluded),true);assert.equal(Number.isInteger(results.total_exposed_units),true);assert.equal(["ready","unavailable"].includes(results.status),true);
  const baseUrl=await standaloneServer();
  ...
  const metadata={experiment:"wanted_landing_v1",analysis_cohort:"wanted_landing_v1-C1",unit_id:unit,variant:assignment.variant,assignment_mode:"assigned",rotator_version:"0.14-R14",exposure_id:token};
