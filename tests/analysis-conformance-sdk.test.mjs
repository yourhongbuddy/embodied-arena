...
test("standalone analysis runner passes every canonical AC4 vector", () => {
  const report = sdk.runWantedAnalysisConformance(analysisConformancePack);
  assert.equal(report.status, "pass");
  assert.equal(report.sdk_version, "0.2-ACS4");
  assert.equal(report.pack_version, "0.2-AC4");
  assert.equal(report.analysis_profile_version, "0.2-A2");
  assert.equal(report.passed_vectors, 9);
  assert.equal(report.total_vectors, 9);
...
test("standalone runner exposes exact numeric and refusal mismatches", () => {
  ...
  assert.equal(report.passed_vectors, 7);
...
test("standalone runner rejects drifted pack metadata before execution", () => {
  ...
  drifted.vectors[0].id = "AC4-SUBSTITUTE";
  ...
  assert.match(report.metadata_errors.join(" "), /canonical AC4 vector/);
});
...
test("published runner and contract are zero-dependency and digest-bound", async () => {
  ...
  assert.equal(ANALYSIS_CONFORMANCE_SDK_VERSION, "0.2-ACS4");
